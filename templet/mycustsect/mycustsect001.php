	...
	<tr>
		...
		<TR>
			...
			<TD><A HREF="<?=$Dir.FrontDir?>mypage_custsect.php"><IMG SRC="<?=$Dir?>images/common/mypersonal_skin3_menu9r.gif" BORDER="0"></A></TD>
	...
	<tr>
		...
		<table border=0 cellpadding=0 cellspacing=0 width=100% style="table-layout:fixed" class="orderlistTbl">
		<col width=30></col>
		<col width=200></col>
		<col width=180></col>
		<col width=></col>
...
<?
		...
		while($row=mysql_fetch_object($result)) {
			...
			echo "<tr>\n";
			echo "	<td align=center  style=\"padding:7,0\"><input type=checkbox class=\"checkbox\" name=sels value=\"".$row->vender."\" style=\"BORDER:none;\"></td>\n";
			...
			if(!_empty($venderinfo['com_image'])){
				...
				$ratioh = ($size[1] > 80)?(real)((185*100)/$size[0]):1;
			...
			echo "	<tr>\n";
			echo "		<td align=center style=\"border:1px #ededed solid\"><a href=\"javascript:GoMinishop('".(MinishopType=="ON"?$Dir."minishop/".$row->id:$Dir."minishop.php?storeid=".$row->id)."')\"> <img src=\"".$logo."\" width=\"".$width."\" height=\"".$height."\" border=0></a></td>\n";
			echo "	</tr>\n";
			...
			echo "	</table>\n";
			echo "	</td>\n";
			echo "	<td align=center style=\"padding:25,10\"><B>".$row->brand_name."</B></td>\n";
			if($row->hot_used=="1") {
				echo "	<td >\n";
				...
				while($row2=mysql_fetch_object($result2)) {
					echo "<td width=80 align=center style=\"padding:7,0\">\n";
					echo "<table border=0 cellpadding=0 cellspacing=0 style=\"table-layout:fixed\">\n";
					echo "<tr>\n";
				...
				echo "	<td align=center  style=\"padding:15,10\">HOT 추천상품이 없습니다.</td>\n";
			}
			echo "</tr>\n";
			$cnt++;
		}
		mysql_free_result($result);
		if ($cnt==0) {
			echo "<tr height=40><td colspan=4 align=center style=\"padding:30px;\">등록된 단골매장이 없습니다.</td></tr>";
		}
...
?>
		<tr>
			<td colspan=4 align=center style="border-bottom:0px;">
