...
<div style="padding:10px 30px;background:#ffffff url('/data/design/img/main/bg_boxline.gif') repeat-y;overflow:hidden;">
	<div style="float:left;padding-left:50px;height:35px;line-height:35px;background:url('/data/design/img/sub/icon_basket.gif') no-repeat;color:#696969;font-size:25px;font-weight:600;">주문완료</div>
...
<tr>
<!-- 여기-->
	<td align="center"><img src="<?=$Dir?>images/common/order/<?=$_data->design_order?>/orderend_img_error.gif" border="0" alt="결제 실패"></td>
</tr>
<tr>
	<td align="center" height=20></td>
</tr>
...
<?
	$sql = "SELECT r.*,p.* FROM tblorderproduct p left join rent_schedule r using(ordercode,basketidx) WHERE ordercode='".$ordercode."' ORDER BY productcode ASC ";
	...
	unset($etcdata);
	unset($prdata);
	$longdiscountitems = array();
	while($row=mysql_fetch_object($result)) {
		$longdiscount += $row->longdiscount;
		if(abs($row->longdiscount) > 0){
			array_push($longdiscountitems,$row->productname.'('.$row->opt1_name.'/'.$row->opt2_name.')');
		}
		$optvalue="";
		...
		if(ereg("^(COU)([0-9]{8,9})(X)$",$row->productcode)) {				#쿠폰
			$etcdata[]=$row;
			continue;
		...
		$sumprice= $row->price*$row->quantity - $row->longdiscount;
		$totprice+=$sumprice;
		echo "<tr ".($isnot==false?"height=\"22\" bgcolor=\"#F2FAFF\"":"height=\"26\"").">\n";
		echo "	<td ".($isnot==false?"align=\"right\" style=\"word-break:break-all;\"":"style=\"padding-left:5px;word-break:break-all;\"").">";
		echo ($isnot==false?"<font color=\"#0000FF\"><b>".viewselfcode($row->productname,$row->selfcode)."</B></font><b>&nbsp:&nbsp;</b>":"<font color=\"#000000\"><B>".viewselfcode($row->productname,$row->selfcode)."</B></font>");
		echo "	</td>\n";
		echo "	<td align=\"center\"><font color=\"#000000\">".($isnot==true?$row->quantity:"&nbsp;")."</font></td>\n";
		echo "	<td align=\"right\" style=\"padding-right:5px\"><font color=\"#000000\">".number_format($sumreserve)."원</font></td>\n";
...
<?
	...
	for($i=0;$i<count($etcdata);$i++) {
		...
		if(ereg("^(COU)([0-9]{8,9})(X)$",$etcdata[$i]->productcode)) {				#쿠폰
			$etcprice+=$etcdata[$i]->price;
			$etcreserve+=$etcdata[$i]->reserve;
			$stateMsg = ( $etcdata[$i]->reserve > 0)?"쿠폰 적립":"쿠폰 할인";
			echo "<tr height=\"25\">\n";
			echo "	<td align=\"center\"><b>".$stateMsg."</b></td>\n";
	...
	if($_ord->dc_now>0) {
		echo "<tr><td colspan=\"5\" height=\"1\" bgcolor=\"#DDDDDD\"></td></tr>\n";
		echo "<tr height=\"25\">\n";
		echo "	<td align=\"center\"><b>즉시할인</b></td>\n";
		echo "	<td style=\"padding-left:5px;word-break:break-all;\">적립예정금액 ".number_format($_ord->dc_now)."원 사용</td>\n";
		echo "	<td align=\"right\" style=\"padding-right:5px;\"><font color=\"#000000\">-".number_format($_ord->dc_now)."원</font></td>\n";
		echo "	<td align=\"right\" style=\"padding-right:5px;\"><font color=\"#FF3C00\"><b>&nbsp;</b></font></td>\n";
		echo "	<td style=\"padding-right:5px;word-break:break-all;\" align=\"right\"><font color=\"#000000\"><B>주문서 전체 적용</b></font></td>\n";
		echo "</tr>\n";
	}
	...
	if($longdiscount<>0) {	?>
		...
		<tr height="25">
			<td align="center"><b>할인</b></td>
			<td style="padding-left:5px;word-break:break-all;">장기대여할인</td>
			<td align="right" style="padding-right:5px;"><font color="#000000"><?=number_format($longdiscount)?>원</font></td>
			<td align="right" style="padding-right:5px;"><font color="#FF3C00">0</td>
			<td style="padding-right:5px;word-break:break-all;" align="right"><?=implode(',',$longdiscountitems)?></td>
		</tr>
<?	}
	
?>
			<tr height="26" bgcolor="#FBFBFB">
				<td colspan="5" align="right" style="padding-right:5px;"><font color="#000000"><B>합계&nbsp:&nbsp;</b></font><font color="#FF3C00"><b><?=number_format($_ord->deli_price-$salemoney-$_ord->reserve-$_ord->dc_now+$etcprice+$longdiscount)?>원</b></font></td>
			</tr>
		...
		<tr>
			...
			<tr height="30" bgcolor="#FBFBFB">
				<td align="right" style="padding-right:5px;"><font color="#000000"><B>최종 결제금액&nbsp:&nbsp;</b></font></td>
	...
	<tr>
		...
		<TR>
			...
			<tr>
				<td style="padding:10px;text-align:left;" height="100%">
...
<?
	if(preg_match("/^(V|C|P|M){1}/", $_ord->paymethod)) {
		...
		?>
				결제성공 - 승인번호 : <?=$_ord->pay_auth_no?>
		<?	
			} else {
		?>
				<font color=blue>정상처리</font> 되었습니다.
		<?
			}
...
<?
	for($i=0;$i<count($prdata);$i++) {
		if(strlen($prdata[$i]->order_prmsg)>0 && substr($prdata[$i]->productcode,0,3) != 'COU') {
	...
	<tr>
		...
		<td>
			...
			<tr>
				...
				<tr><td><b>선물 받는 친구에게 메일이 전송되었습니다.</b></td>
				</tr>
				<tr>
