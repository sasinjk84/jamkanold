	...
	<tr>
		...
		?>
			<ul class="prSortType">
				...
				<li><a href="javascript:ChangeSort('best_desc');" <?=$_sellcount_desc?>>인기상품순</a></li>
...
<?
		...
		while($row=mysql_fetch_object($result)) {
			...
			echo "<tr>";
			echo "	<TD style=\"padding:5px 7px; word-break:break-all;\"><A HREF=\"".$Dir.FrontDir."productdetail.php?".$brand_link."productcode=".$row->productcode.$add_query."&sort=".$sort."\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><FONT class=\"prname\">".viewproductname($row->productname,$row->etctype,$row->selfcode)."</FONT>".(strlen($row->prmsg)?'<br /><span class="prmsgArea">'.$row->prmsg.'</span>':'')."</A></td>\n";
			echo "</tr>\n";
			...
			if($dicker=dickerview($row->etctype,number_format($row->sellprice)."원",1)) {
				...
				echo "<strong class=\"".$mainprpriceClass."\">".$wholeSaleIcon.number_format($row->sellprice)."</strong><strong>원</strong>";
				//echo "<img src=\"".$Dir."images/common/won_icon.gif\" border=0 style=\"margin-right:2px;\">".$wholeSaleIcon.number_format($row->sellprice)."원";
			...
			if( $memberprice > 0 ) {
				...
				//echo "	<td align=center valign=top style=\"word-break:break-all;\" class=\"mainprprice\"><img src=\"".$Dir."images/common/memsale_icon.gif\" align=\"absmiddle\" alt=\"\" />".dickerview($row->etctype,$memberprice."원")."</td>\n";
			}
