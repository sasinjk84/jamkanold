...
?>
	<tr>
		...
		<tr>
			...
			<td width="100%" background="<?=$Dir?>images/common/product/<?=$_cdata->list_type?>/plist_skin_stibg.gif" style="color:#666666;font-size:11px;"><B><?=$_cdata->code_name?></B> 총 등록상품 : <b><?=$t_count?>건</b></td>
...
<?
			...
			while($row=mysql_fetch_object($result)) {
				...
				if($i>0 && $i%3==0) echo "</tr><tr><td height=\"5\"></td></tr><tr><td colspan=\"3\" height=\"1\" background=\"".$Dir."images/common/product/".$_cdata->list_type."/plist_skin_table01.gif\"></td></tr><tr><td height=\"5\"></td></tr><tr><td height=\"5\"></td></tr><tr>\n";
?>
			<td align="center" width="33%" valign="top">
			<table cellpadding="0" cellspacing="0" width="100%" id="G<?=$row->productcode?>" onmouseover="quickfun_show(this,'G<?=$row->productcode?>','')" onmouseout="quickfun_show(this,'G<?=$row->productcode?>','none')">
			<tr>
				<td style="word-break:break-all;"><a href='<?=$Dir.FrontDir?>productdetail.php?productcode=<?=$row->productcode.$add_query?>&sort=<?=$sort?>' onmouseover="window.status='상품상세조회';return true;" onmouseout="window.status='';return true;"><font color="#000000" style="font-size:11px;letter-spacing:-0.5pt;"><b><?=viewselfcode($row->productname,$row->selfcode)?></b></font></a></td>
			</tr>
			<tr>
				<td>
				<table cellpadding="0" cellspacing="0" width="100%">
...
<?
						...
						if (strlen($row->tinyimage)>0 && file_exists($Dir.DataDir."shopimages/product/".$row->tinyimage)==true) {
							...
							echo "<img src=\"".$Dir."images/no_img.gif\" border=\"0\" align=\"center\" width=\"80\" height=\"80\"";
						}
						echo "	></A></td>";
?>
					</tr>
					<tr>
						<td height="3" style="position:relative;"><?=($_data->ETCTYPE["QUICKTOOLS"]!="Y"?"<script>quickfun_write('".$Dir."','G','".$row->productcode."','".($row->quantity=="0"?"":"1")."')</script>":"")?></td>
					</tr>
					<tr>
						<td align="center" style="word-break:break-all;"><A HREF="<?=$Dir.FrontDir?>productdetail.php?productcode=<?=$row->productcode.$add_query?>&sort=<?=$sort?>" onmouseover="window.status='상품상세조회';return true;" onmouseout="window.status='';return true;"><IMG SRC="<?=$Dir?>images/common/product/<?=$_cdata->list_type?>/plist_skin_gong_view.gif" border="0" vspace="3"></a></td>
					</tr>
					</table>
					</td>
					<td></td>
					...
					<table cellpadding="0" cellspacing="0" width="100%">
					<tr>
						<td style="font-size:11px;word-break:break-all;"><IMG SRC="<?=$Dir?>images/common/product/<?=$_cdata->list_type?>/plist_skin_communitynero.gif" border="0"> 시중가 : <s><?=number_format($row->consumerprice)?>원</s></td>
					</tr>
					<tr>
						<td style="font-size:11px;word-break:break-all;"><IMG SRC="<?=$Dir?>images/common/product/<?=$_cdata->list_type?>/plist_skin_communitynero.gif" border="0"> 현재가 : <font color="#F02800" style="font-size:11px;letter-spacing:-0.5pt;"><b><?=number_format($row->sellprice)?>원</b></font></td>
					</tr>
					<?$reserveconv=getReserveConversion($row->reserve,$row->reservetype,$row->sellprice,"Y"); if($reserveconv>0){?>
					<tr>
						<td style="font-size:11px;word-break:break-all;"><IMG SRC="<?=$Dir?>images/common/product/<?=$_cdata->list_type?>/plist_skin_communitynero.gif" border="0"> 적립금 : <?=number_format($reserveconv)?>원</td>
					</tr>
					<?}?>
					<tr>
						<td style="font-size:11px;word-break:break-all;"><IMG SRC="<?=$Dir?>images/common/product/<?=$_cdata->list_type?>/plist_skin_communitynero.gif" border="0"> 남은수량 : 
						<?
					...
					<tr>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" width="100%">
							<tr align="center">
								<td width="43" height="40" align="center" valign="top" style="font-size:11px;"><?=number_format($row->consumerprice)?></td>
						...
						<tr>
							...
							<table cellpadding="0" cellspacing="0" width="100%">
							<tr>
								<td width="43" align="right" style="font-size:11px;">시작가</td>
								<td width="43" align="right" style="font-size:11px;">공구가</td>
							</tr>
							</table>
							</td>
						</tr>
				...
				for($ii=0;$ii<$plist0_tag_0_count;$ii++) {
					...
					if(strlen($taglist[$ii])>0) {
						if($jj==0) {
							...
							echo "	<img src=\"".$Dir."images/common/tag_icon.gif\" border=\"0\" align=\"absmiddle\" style=\"margin-right:2px;\"><a href=\"".$Dir.FrontDir."tag.php?tagname=".urlencode($taglist[$ii])."\" onmouseover=\"window.status='".$taglist[$ii]."';return true;\" onmouseout=\"window.status='';return true;\">".$taglist[$ii]."</a>";
						}
						else {
...
<?
			...
			mysql_free_result($result);
			if($i>0 && $i!=3) echo "<td width=\"33%\"></td>";
			if($i>0 && $i!=2) echo "<td width=\"33%\"></td>";
?>
		</tr>
		</table>
		</td>
