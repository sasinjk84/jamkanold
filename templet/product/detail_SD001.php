				...
				if($multi_img=="Y") {
					...
					if($_pdata->img_type==1) {
						if(strlen($_pdata->minimage)>0 && file_exists($Dir.DataDir."shopimages/product/".$_pdata->minimage)) {
							...
							echo "<div style=\"width:{$width[0]}px; height:{$width[1]}px;background:url(".$Dir.DataDir."shopimages/product/".urlencode($_pdata->minimage).");text-align:center;padding-top:80px;\" ><b><font color=white size=6>".number_format($_pdata->consumerprice)."원</b></div>";
						}
						else {
...
if(eregi("S",$_cdata->type)) {
	...
	if($_pdata->sell_startdate<= $nowTime && $nowTime <= $_pdata->sell_enddate){
		if($_pdata->quantity != null && $_pdata->quantity == 0){
			...
			$strLeftDay = ($left_d>0)? "<td width=\"60\" align=\"center\"><span style=\"font-family:Verdana;font-weight:bold; font-size:20px; color:#000;\" id=\"timeleft0\">".$left_d."일</span></td>":"";

			//$sellstateBtn = "<a href=\"javascript:CheckForm('ordernow4','".$opti."')\" onMouseOver=\"window.status='바로구매';return true;\"\"><img src=\"".$Dir."images/common/product/".$_cdata->detail_type."/pdetail_skin_btn01.gif\" border=0></a>";
			$sellstateBtn = "<a href=\"javascript:CheckForm('ordernow4','".$opti."')\" onMouseOver=\"window.status='바로구매';return true;\"\"><img src=\"/data/design/img/detail/btn_baro.gif\" border=0></a>";
		}
	}else{
		$odrChk = false;
	...
	if($_pdata->discount_state == "Y"){
		$discountRate = sprintf("<font color=\"#FE6700\">%d명이상 구매하면 할인율 %s</font> "
	...
	if(strlen($_pdata->option1)>0){
		...
		$prOption .="<option value=\"\">옵션을 선택하세요</option>\n";
		$prOption .="<option value=\"\">-----------------</option>\n";
		for($i=1;$i<$count;$i++)
		{
			$optPrice = ($arOptPrice)? " (".$arOptPrice[$i-1]."원)":"";
			if(sizeof($arOption2))
			{
				for($k=1;$k<sizeof($arOption2);$k++)
				{
					...
					if($_pdata->stock_state == "Y"){
						$optCnt	= " - 수량:".(($arOptCnt[$posCnt-1]>0)? $arOptCnt[$posCnt-1]."개":(($arOptCnt[$posCnt-1]=="0")? "품절":"무제한"));
					}
				...
				if($_pdata->stock_state == "Y" && sizeof($arOptCnt) > 0){
					$optCnt	= " - 수량:".(($arOptCnt[$i-1]>0)? $arOptCnt[$i-1]."개":(($arOptCnt[$i-1]=="0")? "품절":"무제한"));
				}
					...
					<td>
						...
						<tr>
							<td colspan=9>
								...
								<table cellpadding="0" cellspacing="0" width=100%>
									<tr>
										<td height=25><font color="#6D6D6D" style="font-size:11px;letter-spacing:-0.5pt;word-break:break-all;">정상가격</font></td>
										<td align=right><strike><?=number_format($_pdata->consumerprice)?>원</strike></td>
									</tr>
									<tr>
										<td height=25><font color="#6D6D6D" style="font-size:11px;letter-spacing:-0.5pt;word-break:break-all;">할인가</font></td>
										<td align=right><?=$discountRate?><b><span id="idx_price"><?=number_format($_pdata->sellprice)?></span>원</b></td>
									</tr><?if($reserveconv){?>
									<tr>
										<td height=25><font color="#6D6D6D" style="font-size:11px;letter-spacing:-0.5pt;word-break:break-all;">적립금</font></td>
										<td align=right><?=$reserveconv?>원</td>
									</tr><?}?>
									<tr>
										<td height=25><font color="#6D6D6D" style="font-size:11px;letter-spacing:-0.5pt;word-break:break-all;">수량</font></td>
										<td align=right>
...
?>
	...
	function showCountdown(){
		if(leftTime>0){
			...
			if(day >0){
				document.getElementById("timeleft0").innerText = day+"일";
			}
			...
			if(eregi("S",$_cdata->type)) {
				...
				if(substr($productcode,0,3)=='999') {
					if(strlen($_pdata->quantity)>0 && $_pdata->quantity<=0)
						...
						echo "<a href=\"javascript:CheckForm('ordernow2','".$opti."')\" onMouseOver=\"window.status='선물하기';return true;\"><img src=../images/design/happycopon_btn02.gif></a>\n";
						echo "<a href=\"javascript:CheckForm('ordernow3','".$opti."')\" onMouseOver=\"window.status='본인구매';return true;\"><img src=../images/design/happycopon_btn01.gif></a>\n";
					}
				}
...
?>
		...
		<tr>
			<td>
				<h4 style="padding-bottom:10px; color:#444444; letter-spacing:-1px;">쿠폰 다운로드</h4>
				<?=$coupon_body?>
			</td>
		</tr>
		<?}?>
		...
		<tr>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%" class="detailTabLink">
				<tr>
					<td class="prdetailTabOn"><span><a href="#1">제품상세정보</a></span></td>
					<td class="prdetailTabOff3"><a href="#2">관련상품</a></td>
					<td class="prdetailTabOff" ><a href="#3">고객상품평<!-- ([REVIEW_TOTAL])--></a></td>
					<td class="prdetailTabOff"><a href="#4">상품Q&A<!-- ([QNA_COUNT])--></a></td>
					<td class="prdetailTabOff"><a href="#6">SNS로 소문내기<!--([PROD_SNS_CNT])--></a></td>
					<td class="prdetailTabOff2"><a href="#5">배송/교환/환불</a></td>
				</tr>
			</table>
			<table cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed">
			<tr>
...
<?
	...
	if($_data->coll_loc=="1") {
		...
		echo "
						...
						<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\" class=\"detailTabLink\">
							<tr>
								<td class=\"prdetailTabOff\"><a href=\"#1\">제품상세정보</a></td>
								<td class=\"prdetailTabOn\"><a href=\"#2\">관련상품</a></td>
								<td class=\"prdetailTabOff3\"><a href=\"#3\">고객상품평</a></td>
								<td class=\"prdetailTabOff\"><a href=\"#4\">상품Q&A</a></td>
								<td class=\"prdetailTabOff\"><a href=\"#6\">SNS로 소문내기</a></td>
								<td class=\"prdetailTabOff2\"><a href=\"#5\">배송/교환/환불</a></td>
							</tr>
						</table>
		";
			...
			<tr>
				<td width="100%">
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%" class="detailTabLink">
						<tr>
							<td class="prdetailTabOff"><a href="#1">제품상세정보</a></td>
							<td class="prdetailTabOff"><a href="#2">관련상품</a></td>
							<td class="prdetailTabOn" ><a href="#3">고객상품평<!-- ([REVIEW_TOTAL])--></a></td>
							<td class="prdetailTabOff3"><a href="#4">상품Q&A<!--  ([QNA_COUNT])--></a></td>
							<td class="prdetailTabOff"><a href="#6">SNS로 소문내기<!-- ([PROD_SNS_CNT])--></a></td>
							<td class="prdetailTabOff2"><a href="#5">배송/교환/환불</a></td>
						</tr>
					</table>
				...
				<td>
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%" class="detailTabLink">
						<tr>
							...
							<td class="prdetailTabOff"><a href="#2">관련상품</a></td>
							<td class="prdetailTabOff"><a href="#3">고객상품평<!-- ([REVIEW_TOTAL])--></a></td>
							<td class="prdetailTabOn"><a href="#4">상품Q&A<!--  ([QNA_COUNT])--></a></td>
							<td class="prdetailTabOff3"><a href="#6">SNS로 소문내기<!--  ([PROD_SNS_CNT])--></a></td>
							<td class="prdetailTabOff2"><a href="#5">배송/교환/환불</a></td>
						</tr>
					</table>
				</td>
						...
						<td>
							...
							<table border="0" cellpadding="0" cellspacing="0" width="100%" class="detailTabLink">
								<tr>
									...
									<td class="prdetailTabOff"><a href="#2">관련상품</a></td>
									<td class="prdetailTabOff" ><a href="#3">고객상품평<!-- ([REVIEW_TOTAL])--></a></td>
									<td class="prdetailTabOff"><a href="#4">상품 Q&A<!--  ([QNA_COUNT])--></a></td>
									<td class="prdetailTabOn"><a href="#6">SNS로 소문내기<!-- ([PROD_SNS_CNT])--></a></td>
									<td class="prdetailTabOff2"><a href="#5">배송/교환/환불</a></td>
								</tr>
							</table>
						</td>
		...
		echo "
						...
						<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\" class=\"detailTabLink\">
							<tr>
								<td class=\"prdetailTabOff\"><a href=\"#1\">제품상세정보</a></td>
								<td class=\"prdetailTabOff\"><a href=\"#2\">관련상품</a></td>
								<td class=\"prdetailTabOff\"><a href=\"#3\">고객상품평</a></td>
								<td class=\"prdetailTabOff\"><a href=\"#4\">상품 Q&A</a></td>
								<td class=\"prdetailTabOff\"><a href=\"#6\">SNS로 소문내기</a></td>
								<td class=\"prdetailTabOn\"><a href=\"#5\">배송/교환/환불</a></td>
							</tr>
						</table>
		";
		echo "	</td>\n";
