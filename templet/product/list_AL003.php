		...
		if($special_1_type == "I") {
			...
			foreach($res as $i=>$row){
				...
				if($dicker=dickerview($row->etctype,number_format($row->sellprice)."원",1)) {
					...
					$sp_list.= "<img src=\"".$Dir."images/common/won_icon.gif\" border=0 style=\"margin-right:2px;\">".$strikeStart.number_format($row->sellprice)."원".$strikeEnd;
					...
					if (strlen($row->option_price)==0) $sp_list.= $strikeStart.number_format($row->sellprice)."원".$strikeEnd;
					else $sp_list.= ereg_replace("\[PRICE\]",number_format($row->sellprice),$_data->proption_price);
				}
				
				//회원할인가 적용
				if($row->discountprices>0){
					$sp_list.= '<br>'.number_format($memberprice)."원";
				}
			...
			foreach($res as $i=>$row){
				...
				if($dicker=dickerview($row->etctype,number_format($row->sellprice)."원",1)) {
					...
					$sp_list.= "<br><font class=\"prprice\"><img src=\"".$Dir."images/common/won_icon.gif\" border=\"0\" style=\"margin-right:2px;\">".$strikeStart.number_format($row->sellprice)."원".$strikeEnd;
					if (strlen($row->option_price)!=0) $sp_list.= "(기본가)";
					$sp_list.= "</font>";
				} else {
...
<?
		...
		while($row=mysql_fetch_object($result)) {
			...
			echo "<tr>";
			echo "	<TD align=\"center\" style=\"word-break:break-all;\"><A HREF=\"".$Dir.FrontDir."productdetail.php?productcode=".$row->productcode.$add_query."&sort=".$sort."\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><FONT class=\"prname\">".viewproductname($row->productname,$row->etctype,$row->selfcode)."</FONT>".(strlen($row->prmsg)?'<br><span class="prmsgArea">'.$row->prmsg.'</span>':'')."</A></td>\n";
			echo "</tr>\n";
			if($row->consumerprice!=0) {
				echo "<tr>\n";
			...
			if($dicker=dickerview($row->etctype,number_format($row->sellprice)."원",1)) {
				...
				echo "<img src=\"".$Dir."images/common/won_icon.gif\" border=0 style=\"margin-right:2px;\">".$strikeStart.number_format($row->sellprice)."원".$strikeEnd;
				...
				if (strlen($row->option_price)==0) echo $strikeStart.number_format($row->sellprice)."원".$strikeEnd;
				else echo ereg_replace("\[PRICE\]",number_format($row->sellprice),$_data->proption_price);
			}
			
			//회원할인가 적용
			if($discountprices>0){
				echo '<br>'.number_format($memberprice)."원";
			}
