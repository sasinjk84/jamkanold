...
<TABLE border=0 CELLSPACING=0 CELLPADDING=0 BGCOLOR="<?=$comment_header_bg_color?>" style="margin-bottom:25px; TABLE-LAYOUT:FIXED">
				...
				<TR>
					<TD style="font-size:11px; letter-spacing:-0.5pt; padding:5px 10px;" bgColor="#fafafa">
						<? if (strlen($member[name])>0) { ?>
							작성자 : <?= $member[name] ?><input type=hidden name="up_name" value="<?=$member[name]?>">
						<? } else { ?>
							작성자 : <input type=text name="up_name" size="13" maxlength="10" value="" class="input" />
							<img width="20" height="0">비밀번호 : <INPUT type=password name="up_passwd" value="" maxLength="20" size="10" class="input" />
						<? } ?>
					</TD>
				</TR>
				<TR bgColor="#fafafa" align="center">
					<TD>
						<TABLE border="0" cellSpacing="0" cellPadding="0" width="100%" style="table-layout:fixed">
							...
							<col width="100"></col>
							<tr>
								<td style="padding:5px;"><textarea name=up_comment style="width:100%; height:70px;line-height:17px;border:solid 1;border-color:#BDBDBD;font-size:9pt;color:333333;background-color:white;"></textarea></td>
	...
	<? if($setup[fileYN] == "Y"){ ?>
	<tr>
		<td style="font-size:11px; letter-spacing:-0.5pt; padding:5px 10px;" bgColor="#fafafa">파일첨부 :&nbsp;<?=$cmtFile?></td>
	</tr>
	<? } ?>
