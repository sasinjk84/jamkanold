			...
			<TABLE cellSpacing="0" cellPadding="0" border="0" width="100%">
				...
				<TR>
					...
					<td align="right">
						...
						<? if( $setup["onlyCmt"] == "N" OR strlen($_ShopInfo->id) > 0 ){ ?>&nbsp;<IMG src="<?=$imgdir?>/board_del.gif" align="absmiddle" border="0" style="CURSOR:hand;" onclick="location='board.php?pagetype=delete_comment&board=<?=$board?>&num=<?=$num?>&c_num=<?=$c_num?>&s_check=<?=$s_check?>&search=<?=$search?>&block=<?=$block?>&gotopage=<?=$gotopage?>'"><? } ?>
					</TD>
				</TR>
				<TR>
