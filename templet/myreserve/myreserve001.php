...
<tr>
	<td>
				<div class="orderStateWrap" style="text-align:center;padding:26px 0px;"><span style="font-size:20px;color:#777777;padding:0px 10px 0px 0px">사용가능한 적립금</span><span style="color:#FF6600;font-weight:bold;font-size:30px;"><?=number_format($reserve)?>원</div>
	<table cellpadding="0" cellspacing="0" width="100%">
	<!--
	<tr>
		<td>
...
<?
		...
		while($row=mysql_fetch_object($result)) {
			...
			if(strlen($orderprice)>0 && $orderprice>0) {
				echo "<font color=\"#F02800\"><b>".number_format($orderprice)."원";
			} else {
				echo "-";
			}
			echo "</td>\n";
			echo "	<td class=\"centerCell\">".number_format($row->reserve)."원</td>\n";
			echo "</tr>\n";
			$cnt++;
		}
		mysql_free_result($result);
		if ($cnt==0) {
			echo "<tr><td colspan=\"4\" align=\"center\" style=\"padding:30px;\">해당내역이 없습니다.</td></tr>";
		}
?>
		</table>
		</td>
