...
<table cellpadding="0" cellspacing="0" width="100%" style="table-layout:fixed">
	...
	<tr>
		<td height="35">
			...
			?>
			<ul class="prSortType">
				<li><a href="javascript:ChangeSort('new_desc');" <?=$_new?>>신규등록순</a></li>
		...
		*/
		...
		//foreach($res as $i=>$row){
			...
			if($row->consumerprice!=0) {
				echo "	<span class=\"mainconprice\"><strike>".number_format($row->consumerprice)."</strike>원</span>\n";
			...
			if($dicker=dickerview($row->etctype,$wholeSaleIcon.number_format($row->sellprice)."원",1)) {
				...
				if (strlen($row->option_price)==0)
					echo "<img src=\"".$Dir."images/common/won_icon.gif\" border=0>".number_format($row->sellprice)."원";
				else
			...
			*/
			...
			if($reserveconv>0) {
				echo "	<div style=\"margin-top:2px;\"><img src=\"".$Dir."images/common/reserve_icon.gif\" border=\"0\" style=\"margin-right:2px;\" align=\"absmiddle\" alt=\"\" /><span class=\"prreserve\">".number_format($reserveconv)."</span>원</div>\n";
