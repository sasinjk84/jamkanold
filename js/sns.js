function showDiv(showID){
	...
	if(preShowID != showID){
		...
		if($j("#"+showID).length > 0) {
			//
			
			$j("#"+showID).css({visibility:'visible'});
			
			$j("#m_snsSend").attr("onClick","snsReg_top('f');");
	 ...
	 function(data){
		if (data.result == 'true') {
			if (data.me2day == undefined ) {
				$j("#mLoginBtnChk").val("NON");
				setImgChange("m");
			}else if ( data.me2day == "N") {
				$j("#mLoginBtnChk").val("N");
				setImgChange("m");
			}else if ( data.me2day == "Y") {
				$j("#mLoginBtnChk").val("Y");
				setImgChange("m");
			}
