	...
	function startAction() {
		...
		// 빌링 승인	
		  ...
		  $this->m_uri = "/lite/billingProcess.jsp";
		   ...
		   $NICELog->WriteLog($this->m_queryString["BillKey"] );
		...
		//request
		  if( !$httpclient->HttpRequest($this->m_uri, $this->m_queryString, $NICELog) ) {
			...
			if( $httpclient->getErrorCode() == READ_TIMEOUT_ERR )
			{
  			...
  			$this->m_uri = "/lite/cancelProcess.jsp";
  			unset($this->m_queryString);
  			$this->m_queryString["MID"] = substr($_POST["TID"], 0,10);
  			$this->m_queryString["TID"] = $_POST["TID"];
		...
		if ( $httpclient->getStatus() == "200" ) {   
			...
			if( $httpclient->getStatus() != 200 )
			{
				...
				$this->m_uri = "/lite/cancelProcess.jsp";
				unset($this->m_queryString);
				$this->m_queryString["MID"] = $_POST["MID"];
				$this->m_queryString["TID"] = $_POST["TID"];
				$this->m_queryString["CancelAmt"] = $_POST["Amt"];
				$this->m_queryString["CancelMsg"] = "NICE_NET_CANCEL";
				$this->m_queryString["CancelPwd"] = "123456";
				$this->m_queryString["NetCancelCode"] = "1";
				...
				if( !$httpclient->HttpRequest($this->m_uri, $this->m_qs, $NICELog) && ($httpclient->getErrorCode() == READ_TIMEOUT_ERR) )
				{
					$NICELog->WriteLog("Net Cancel FAIL" );
					if( $this->m_ActionType == "PYO")
						$this->MakeErrorMsg( ERR_RESPONSE, "승인여부 확인요망"); 
