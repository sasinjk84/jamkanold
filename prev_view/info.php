...
if($type=="view") {
	...
	if($row=mysql_fetch_object($result)) {
		...
		echo "<html><head><title></title></head><body onload=\"alert('해당 정보가 존재하지 않습니다.');location.href='".$list."'\"></body></html>";exit;
	}
	...
	$sql = "SELECT filename,body FROM tbldesignnewpage_prev WHERE type='infolist' ";
	$result=mysql_query($sql,get_db_conn());
	if($row=mysql_fetch_object($result)) {
		$size=explode("",$row->filename);
		$xsize=1024;
