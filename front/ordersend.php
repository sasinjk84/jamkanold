		...
		if($giftinfo=mysql_fetch_assoc($result)){
					...
					if(!_empty($_REQUEST['giftOpt'.$g])){
						if(isset($giftinfo['options'][$g][1][$_REQUEST['giftOpt'.$g]])){
							if($giftinfo['options'][$g][1][$_REQUEST['giftOpt'.$g]] !== true){
								...
								if($chkq < 1){
									echo "<html></head><body onload=\"alert('사은품의 해당 옵션이 다른 고객에 의해 품절 되었습니다. 사은품을 다시 선택해 주세요.');parent.location.href='".$Dir.FrontDir."basket.php'\"></body></html>";
									exit;
								}
							}
			...
			if(_isInt($giftinfo['quantity'])){
				if($giftinfo['quantity'] < 0){
					echo "<html></head><body onload=\"alert('사은품이 다른 고객에 의해 품절 되었습니다. 사은품을 다시 선택해 주세요.');parent.location.href='".$Dir.FrontDir."basket.php'\"></body></html>";
					exit;
				}
			}
		...
		foreach( $vendervalue['products'] as $productKey=>$product ) {
			...
			if($product['rental'] != '2'){ // 렌탈 상품 제외			
				...
				if($prentinfo['codeinfo']['pricetype']=="long"){
					if($tmpp['options'][$product['rentinfo']['optidx']]['optionPay']=="일시납"){
						$orderProductOptValue[1] = $tmpp['options'][$product['rentinfo']['optidx']]['optionPay'];
					}else{
						$orderProductOptValue[1] = $tmpp['options'][$product['rentinfo']['optidx']]['optionPay']." 월 ".number_format($tmpp['options'][$product['rentinfo']['optidx']]['nomalPrice']/$tmpp['options'][$product['rentinfo']['optidx']]['optionName'])."원";
					}
				}else{
...
*/
	...
	// 적립금 사용  --------------------------------------------------------------------------------------------------------------------------------------
	if( $usereserve > 0 ) {
		$usereserve = ( $usereserve > $sumprice ) ? $sumprice : $usereserve;// 적립금이
		$sumprice -= $usereserve;
		$sumprice = ( $sumprice < 0 ) ? 0 : $sumprice;
	...
	$basketItemsCoupon = getBasketByArray($basket,$basketTempArray); // 쿠폰 적용됨
	...
	*/
	...
	foreach($basketItemsCoupon['vender'] as $venderCoupon=>$vendervalueCoupon) {
		...
		if( $vendervalueCoupon['deliprice'] > 0 ) {
			...
			//echo "<hr>배송비 정보 저장 <br> ".$deliSQL;exit;
		}
	}
	// 배송비 정보 저장 끝 --------------------------------------------//
	...
	*/
	...
	/*
	echo "총결제액 : ".$last_price.'<br>';
	echo "과세액 : ".$SupplyAmt.'<br>';
	echo "과세VAT : ".$vat.'<br>';
	echo "면세액 : ".$tax_free.'<br>';
