...
if (preg_match("/^(V|O|Q|C|P|M)$/", $paymethod) && $pay_flag!="0000") {
	mysql_query("UPDATE tblbasket3 SET tempkey='".$_ShopInfo->getTempkey()."' WHERE tempkey='".$_ShopInfo->getGifttempkey()."'",get_db_conn());
} else {
	mysql_query("DELETE FROM tblbasket3 WHERE tempkey='".$_ShopInfo->getGifttempkey()."'",get_db_conn());
}
...
($paymethod=="V" && strlen($ordercode)>0 && $deli_gbn=="C" && $deli_flag=="N" && strlen($bank_date)==0) && strlen($delflag)==0)
{
	...
	$sql.= "FROM tblproduct a, tblbasket3 b WHERE b.tempkey='".$_ShopInfo->getTempkey()."' ";
