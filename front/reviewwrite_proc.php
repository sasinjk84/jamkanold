<?
	...
	if($_data->review_type =="N" || $_data->ETCTYPE["REVIEW"]=="N") {
		echo '<script>alert("사용후기 기능 설정이 되지 않아 사용할 수 없습니다.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}
	if(strlen($_ShopInfo->getMemid())==0 && $_data->review_memtype=="Y"){
		echo '<script>alert("회원전용 기능입니다.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}

	if(strlen($mode) <= 0 || $mode != "write"){
		echo '<script>alert("정상적인 방법이 아니므로 접근할 수 없습니다.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($code) <= 0 || strlen($code)>12){
		echo '<script>alert("정상적인 방법이 아니므로 접근할 수 없습니다2.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($productcode) <= 0 || strlen($productcode)>18){
		echo '<script>alert("정상적인 방법이 아니므로 접근할 수 없습니다3.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($quality) <= 0 || ($quality < 1 || $quality > 5)){
		echo '<script>alert("품질 점수를 선택해 주세요.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($price) <= 0 || ($price < 1 || $price > 5)){
		echo '<script>alert("가격 점수를 선택해 주세요.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($delitime) <= 0 || ($delitime < 1 || $delitime > 5)){
		echo '<script>alert("배송 점수를 선택해 주세요.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($recommend) <= 0 || ($recommend < 1 || $recommend > 5)){
		echo '<script>alert("추천 점수 선택해 주세요.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($writer) <= 0){
		echo '<script>alert("작성자를 입력해 주세요.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}else if(strlen($contents) <= 0){
		echo '<script>alert("내용을 입력해 주세요.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
	}
...
*/
	...
	if(strlen($attechfilename)>0){
		...
		if(!in_array($attechfiletype,$allowfile)){
			echo '<script>alert("첨부 가능한 파일이 아닙니다.\n첨부가능한 파일은 jpg, gif, png입니다.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
		}else{
			if($attechfilesize >$maxfilesize){
				echo '<script>alert("첨부 가능한 파일 용량이 초과 되었습니다.\n최대 첨부가능한 파일용량은 '.$maxfilesize_unit.'입니다.");location.replace("/front/productdetail.php?productcode='.$productcode.'");</script>';exit;
			}else{
