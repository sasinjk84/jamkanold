...
class tempTwitter{
	...
	function __construct(){
		$this->oauth_callback = OAUTH_CALLBACK;//'http://'.$_SERVER['HTTP_HOST'].'/front/twitter/callback..php';
		$this->_getAuthcode();
	...
	//	if(!_empty($this->request['oauth_token']) && !_empty($this->request['oauth_token2'])){
		if($_SESSION['oauth_token'] === $_REQUEST['oauth_token']){
			...
			if (200 == $this->connection->http_code && !_empty($memID)){
				/* If method is set change API call made. Test is called by default. */
				$content = $connection->get('account/verify_credentials');
	...
	function _getAuthcode(){
		...
		$this->request = array();
		if($_data->sns_ok == "Y" && !_empty($memID)){
