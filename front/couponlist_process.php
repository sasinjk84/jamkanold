<?
	...
	if($mode=="coupon" && strlen($coupon_code)==8 ) {	//쿠폰 발급
		if(strlen($_ShopInfo->getMemid())==0) {	//비회원
			echo "<html></head><body onload=\"alert('로그인 후 쿠폰 다운로드가 가능합니다.'); parent.location.href='".$Dir.FrontDir."login.php?chUrl=/front/couponlist.php';\"></body></html>";exit;
		} else {
				...
				if(false !== $cRes = mysql_query($cInfoSql, get_db_conn())){
					...
					if($row->issue_tot_no>0 && $row->issue_tot_no<$row->issue_no+1) {
						...
						if($row->date_start>0) {
							...
							echo $date_start = substr($date,0,10);
							//echo $date_end = date('Ymd',strtotime($date.'+'.abs($row->date_start).' day'));
							echo $date_end = date("Ymd",mktime(0,0,0,substr($date,4,2),substr($date,6,2)+abs($row->date_start),substr($date,0,4)))."23";
						}
						...
						if(!mysql_errno()) {
							...
							echo "<script>alert(\"해당 쿠폰 발급이 완료되었습니다.\\n\\n상품 주문시 해당 쿠폰을 사용하실 수 있습니다.\"); parent.location.replace(\"/front/couponlist.php\");</script>";
						} else {
