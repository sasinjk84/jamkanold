...
if(strlen($_ShopInfo->getMemid())==0) {
	...
	$sql.= "receiver_addr, order_msg, ip, del_gbn, partner_id, loc, gift, device) VALUES (";
	...
	$sql.= "'".$receiver_addr."', '".$order_msg."', '".$ip."', '', '".$_ShopInfo->getRefurl()."', '".$loc."','{$gift}','P')";
	mysql_query($sql,get_db_conn());
	backup_save_sql($sql);
	if (mysql_errno()) {
		sendmail(AdminMail,"[긴급!] INSERT ERROR",getenv("HTTP_HOST")."<br>$sql - ".mysql_error(),"Content-Type: text/plain\r\n");
...
$SupplyAmt = $vat = 0;
$tax_free=0;
$rtnpgtype="&rtnpgtype=2";
if($paymethod!="B") {
	########### 결제시스템 연결 시작 ##########
	include($Dir.FrontDir."paylist2.php");
	exit;
	########### 결제시스템 연결 끝   ##########
}
...
########### 최종 마무리 ###########
include($Dir.FrontDir."payresult2.php");
########### 최종 마무리 끝 ########
?>