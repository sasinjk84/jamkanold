...
if ($_data->ETCTYPE["REVIEW"]!="Y") {
	echo "<html><head><title></title></head><body onload=\"alert('고객상품평 모음 게시판을 이용할 수 없습니다.');location.href='".$Dir."main/main.php';\"></body></html>";exit;
}
...
<HTML>
<HEAD>
	<TITLE><?=$_data->shoptitle?> - 고객 상품평</TITLE>
...
<div class="currentTitle">
	<div class="titleimage">고객 상품평</div>
	<!--<div class="current">홈 &gt; <SPAN class="nowCurrent">고객 상품평 전체보기</span></div>-->
</div>
<!-- 고객 상품평 전체보기 상단 메뉴 -->
...
if ($_data->title_type=="Y" && file_exists($Dir.DataDir."design/reviewall_title.gif")) {
	$review_title="<img src=\"".$Dir.DataDir."design/reviewall_title.gif\" border=0 alt=\"고객 상품평\">";
} else {
	/*
...
$review_list.="			<div class=\"button ".$sallreview."\"><a href=\"javascript:reviewSelect('all');\">전체상품평</a></div>";
$review_list.="			<div class=\"button ".$sbestreview."\"><a href=\"javascript:reviewSelect('best');\">베스트상품평</a></div>";
$review_list.="			<div class=\"button ".$sphotoreview."\"><a href=\"javascript:reviewSelect('photo');\">포토상품평</a></div>";
$review_list.="			<div class=\"button ".$sbasicreview."\"><a href=\"javascript:reviewSelect('basic');\">일반상품평</a></div>";

/*
$review_list.="			<select name=\"reviewselect\" onchange=\"reviewSelect(this.value)\">\n";
$review_list.="				<option value=\"all\"$sallreview>전체상품평</option>\n";
$review_list.="				<option value=\"photo\"$sphotoreview>포토상품평</option>\n";
$review_list.="				<option value=\"basic\"$sbasicreview>일반상품평</option>\n";
$review_list.="				<option value=\"best\"$sbestreview>베스트상품평</option>\n";
$review_list.="			</select>\n";
*/
...
$review_list.="	<col width=80></col>\n";
$review_list.="	<tr>\n";
$review_list.="		<th>번호</th>\n";
$review_list.="		<th>이미지</th>\n";
$review_list.="		<th></td>\n";
$review_list.="		<th>상품명/사용후기</th>\n";
$review_list.="		<th>작성자</th>\n";
if($reviewdate!="N") {
	$review_list.="	<th>작성일</th>\n";
}
$review_list.="		<th>평점</th>\n";
$review_list.="	</tr>\n";
...
//if(strlen($likecode)>=3) {
	...
	while($row=mysql_fetch_object($result)) {
		...
		$review_list.="	<td>\n";
		$review_list.="	<table border=0 cellpadding=0 cellspacing=0 width=100%>\n";
		//$review_list.="	<tr><td style=\"padding-left:5\"><A HREF=\"".$Dir.FrontDir."productdetail.php?productcode=".$row->productcode."\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><FONT COLOR=\"#373737\"><B>".viewselfcode($row->productname,$row->selfcode)."</A> <A HREF=\"".$Dir.FrontDir."productdetail.php?productcode=".$row->productcode."\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><img src=\"".$Dir."images/common/review/btn_reviewprview.gif\" border=0 align=absmiddle></A></td></tr>\n";
		$linkpage="";
		if($reviewlist=="Y") {
		...
		$review_list.="	<tr><td style=\"padding-left:5\">".$linkpage."<FONT COLOR=\"#373737\"><B>".viewselfcode($row->productname,$row->selfcode)."</A> <A HREF=\"".$Dir.FrontDir."productdetail.php?productcode=".$row->productcode."\" onmouseover=\"window.status='상품상세조회';return true;\" onmouseout=\"window.status='';return true;\"><img src=\"".$Dir."images/common/review/btn_reviewprview.gif\" border=0 align=absmiddle></A></td></tr>\n";
	...
	if ($cnt==0) {
		$review_list.="<tr height=30><td class=lineleft colspan=".$colspan." align=center style=\"padding:30px;\">검색된 상품리뷰가 없습니다.</td></tr>";
		$review_list.="\n";
	}
