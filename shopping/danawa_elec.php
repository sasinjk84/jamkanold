...
if($engineval[$engine_num] == "checked") {
	...
	if($total>0)
	{
		...
		while($row=mysql_fetch_object($presult)) {
			...
			echo trim($itemcode)."^".trim($part1).trim($part2).trim($part3).trim($part4)."^".trim($modelname)."^".trim($productcom)."^".trim($GoodsImg_Url)."^".trim($url)."^".trim($price)."^".trim($reserve)."^^^^^^".trim($opendate)."^".trim($BesongBi)."\n";
		}
