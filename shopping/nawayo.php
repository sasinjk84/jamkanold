...
if($engineval[$engine_num] == "checked") {
	...
	if($total>0)
	{
		...
		while($row=mysql_fetch_object($presult)) {
			...
			echo trim($itemcode)."^".trim($part1)."^".trim($part2)."^".trim($part3)."^".trim($part4)."^".trim($modelname)."^".trim($productcom)."^^".trim($url)."^".trim($price)."^".trim($GoodsImg_Url2)."^".trim($BesongBi)."^^<br>\n";
		}
