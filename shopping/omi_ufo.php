...
if($engineval[$engine_num] == "checked") {
	...
	if($total>0)
	{
		...
		echo $_data->shopname."입니다.총(".(int)$total.")건 입니다.\n";
		...
		while($row=mysql_fetch_object($presult)) {
			...
			echo "<p>productcode=";
			echo trim($itemcode)."^".trim($part1)."^".trim($part2)."^".trim($part3)."^".trim($productcom)."^".trim($modelname)."^".trim($url)."^".trim($price)."^".trim($BesongBi)."^".trim($GoodsImg_Url)."^^\n";
		}
