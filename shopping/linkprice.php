...
if($engineval[$engine_num] == "checked") {
	...
	if($total>0)
	{
		...
		while($row=mysql_fetch_object($presult)) {
			...
			echo trim($itemcode)."^".trim($part)."^".trim($productcom)."^".trim($modelname)."^^^".trim($url)."^".trim($GoodsImg_Url2)."^".trim($price)."^".trim($reserve)."^^^^".trim($regdate)."^".trim($BesongBi)."^\n";
		}
