...
결과 리턴 관련 내용은  /sci/pcc/result.php 에서 요청 페이지  또는 파라메터별(addVar 값 기준) 처리를 작성해주셔야 합니다.
**************************************************************************/
...
class sci_pcc{
	...
	var $id = 'SOBJ001'; // bizsiren.com 에 등록된 본인인증 확인 업체 id
	...
	function _retInfo($retInfo){
		...
		$return['name']	= $split_dec_retInfo[0];		//성명
		$return['birYMD'] = $split_dec_retInfo[1];		//성별
		$return['sex'] = $split_dec_retInfo[2];		//생년월일
		$return['fgnGbn'] = $split_dec_retInfo[3];		//내외국인 구분값
		...
		$return['reqNum'] = $split_dec_retInfo[8];		//요청번호
		$return['result'] = $split_dec_retInfo[9];		//본인확인 결과 (Y/N)
		$return['certGb'] = $split_dec_retInfo[10];		//인증수단
		$return['cellNo'] = $split_dec_retInfo[11];		//핸드폰 번호
		$return['cellCorp'] = $split_dec_retInfo[12];		//이동통신사
		$return['certDate'] = $split_dec_retInfo[13];		//검증시간
		$return['addVar'] = $split_dec_retInfo[14];		//추가 파라메터
