...
this.btnList = {
    ...
    "justifyleft":    ['JustifyLeft',          '왼쪽정렬',           'editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',        '가운데정렬',         'editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',         '오른쪽정렬',         'editor_action(this.id)',  'ed_align_right.gif'],
    "orderedlist":    ['InsertOrderedList',    '번호달기',           'editor_action(this.id)',  'ed_list_num.gif'],
    ...
    "backcolor":      ['BackColor',            '글자배경색',         'editor_action(this.id)',  'ed_color_bg.gif'],
    "horizontalrule": ['InsertHorizontalRule', '분리선',             'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           '링크걸기',           'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          '그림삽입',           'editor_action(this.id)',  'ed_image.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
  + '<textarea ID="_' +objname + '_editor" style="width:' +config.width+ '; height:' +config.height+ '; border:1 solid #CCCCCC;PADDING:5" wrap=soft></textarea>';
...
function editor_setmode(objname, mode) {
  ...
  var TextEdit   = '<textarea ID="_' +objname + '_editor" style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; margin-top: -1px; margin-bottom: -1px; border:1 solid #CCCCCC;PADDING:5"></textarea>';
  var RichEdit   = '<iframe ID="_' +objname+ '_editor"    style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; margin-top: -1px; margin-bottom: -1px; border:1 solid #CCCCCC;PADDING:5"></iframe>';
