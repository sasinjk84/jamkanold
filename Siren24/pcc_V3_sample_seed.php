...
<?
	...
	$enc_reqInfo = exec("/home/rental/public_html/Siren24/SciSecuX SEED 1 1 $reqInfo ");//암호화모듈 설치시 생성된 SciSecuX 파일이 있는 리눅스 경로를 설정해주세요.

	//04. 요청정보 위변조검증값 생성
	$hmac_str = exec("/home/rental/public_html/Siren24/SciSecuX HMAC 1 1 $enc_reqInfo ");

	//05. 요청정보 2차암호화
	//데이터 생성 규칙 : "요청정보 1차 암호화^위변조검증값^암복화 확장 변수"
	$enc_reqInfo = $enc_reqInfo. "^" .$hmac_str. "^" ."0000000000000000";
	$enc_reqInfo = exec("/home/rental/public_html/Siren24/SciSecuX SEED 1 1 $enc_reqInfo ");
...
<!--
	...
	function sirenResult(name,mobile,sex,birth,ci){
		$("#name").val(name);
		$("#mobile").val(mobile);
