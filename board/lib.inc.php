...
function getSecret($query,&$row) {
	global $_ShopInfo,$_POST,$setup,$member,$view,$board,$num,$block,$gotopage,$search,$s_check,$boardUserid;

	if ($setup[use_lock]!="N") {
		$result = mysql_query($query,get_db_conn());
		$view_ok = mysql_num_rows($result);
		if (!$view_ok || $view_ok == -1) {
			echo "<html><head><title></title></head><body onload=\"alert('잘못된 경로의 글입니다.\\n\\n다시 확인 하십시오.'); document.location.replace('board.php?pagetype=list&board=$board&s_check=$s_check&search=$search');\"></body></html>";exit;
		}
		...
		if ($view=="1") {
			if ($isSecret || $member[admin]=="SU") {
				...
				if (_empty($boardUserid) ||  $boardUserid != $member[id] ) {
			...
			if (!$isSecret && ($member[admin]!="SU" || _empty($boardUserid) ||  $boardUserid != $member[id] ) ) {
