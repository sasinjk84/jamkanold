...
if($type=="d") {
	...
	$sql.="GROUP BY search ORDER BY cnt DESC LIMIT ".$len;
} else if($type=="m") {
	$date=substr($searchdate,0,6);  
	if ($date==date("Ym")) {
		$sql ="SELECT SUM(cnt) as cnt,search FROM tblcounterkeyword ";
		$sql.="WHERE date LIKE '".$date."%' GROUP BY search ORDER BY cnt DESC LIMIT ".$len;
	} else {
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td height="29" colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td height="28" class="link" align="left" background="images/con_link_bg.gif"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 : 통계분석 &gt; 고객 선호도 분석 &gt; <span class="2depth_select">상품 검색 선호도</span></td>
			</tr>
			</table>
		</td>
	</tr>   
...
<?}?>
			<tr>
				...
				<TR>
					<TD><IMG SRC="images/counter_prsearchprefer_title.gif" ALT=""></TD>
					</tr>
<tr>
...
<?
					...
					for($i=0;$i<$len;$i++){
						...
						echo "	<TD class=\"td_con2a\" style=\"padding-left:5\">".$search[$i]."</td>\n";
			...
			<tr>
				...
				<TR>
					...
					<tr>
						...
						<td><FONT color=#3d3d3d>어떠한 키워드 검색을 통해서 구매할 의사를 보였는지 알수 있습니다.</FONT></td>
					</tr>
					<tr><td colspan=2 height=5></td></tr>
					<tr>
						<td width="20" align="right" valign="top"><img src="images/icon_8.gif" width="13" height="18" border="0"></td>
						<td>
							<FONT color=#3d3d3d>상품의 네이밍 작업이나 어떠한 상품을 찾아 왔는지 확인할수 있습니다.</FONT>
						</td>
					</tr>
					<tr><td colspan=2 height=5></td></tr>
					<tr>
						<td width="20" align="right" valign="top"><img src="images/icon_8.gif" width="13" height="18" border="0"></td>
						<td>
							<FONT color=#3d3d3d>고객이 어떠한 검색어로 상품을 찾는지 안다면 상품명이나 검색어등에 노출빈도가 많은 검색어들을 넣어놓는다면<br>어떠한 판매전략과도 바꿀수 없는 소중한 자료가 될것입니다.</FONT>
						</td>
					</tr>
					</table>
					</TD>
