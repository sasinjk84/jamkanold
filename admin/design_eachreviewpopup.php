...
if ( $type=="store" OR $type=="restore" ) {
	$MSG = adminDesingBackup ( $type, 'reviewopen', $body, $subject, '', $filename );
...
if($type=="update" && strlen($body)>0) {
	...
	if($row->cnt==0) {
		...
		$sql.= "filename	= '".$filename."', ";
		$sql.= "body		= '".$body."' ";
		$sql.= "WHERE type='reviewopen' ";
		mysql_query($sql,get_db_conn());
	}
	mysql_free_result($result);
	$onload="<script>alert(\"상품리뷰화면 디자인 수정이 완료되었습니다.\");</script>";
} else if($type=="delete") {
	$sql = "DELETE FROM tbldesignnewpage WHERE type='reviewopen' ";
	mysql_query($sql,get_db_conn());
	$onload="<script>alert(\"상품리뷰화면 디자인 삭제가 완료되었습니다.\");</script>";
} else if($type=="clear") {
	$body="";
	$xsize=450;
	$ysize=400;
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td height="29" colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td height="28" class="link" align="left" background="images/con_link_bg.gif"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 : 디자인관리 &gt; 개별디자인-페이지 본문 &gt; <span class="2depth_select">상품리뷰 보기창 꾸미기</span></td>
			</tr>
			</table>
		</td>
	</tr>
    ...
    <tr>
        ...
        <td bgcolor="#ffffff" style="padding:10px">
			...
			<tr>
				...
				<TR>
					<TD><IMG SRC="images/design_eachreviewpop_title.gif" ALT=""></TD>
					</tr>
<tr>
<TD width="100%" background="images/title_bg.gif" height="21"></TD>
			...
			<tr>
				...
				<TR>
					...
					<TD width="100%" class="notice_blue">각 상품의 리뷰에 대한 상세보기 페이지의 디자인을 자유롭게 디자인 하실 수 있습니다.</TD>
			...
			<tr>
				...
				<TABLE WIDTH="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0>
				<TR>
					<TD><IMG SRC="images/design_eachreview_stitle1.gif" WIDTH="250" HEIGHT=31 ALT=""></TD>
			...
			<tr>
				...
				<TR>
					<TD class="table_cell"><img src="images/icon_point2.gif" width="8" height="11" border="0">리뷰창 크기</TD>
			...
			<tr>
				...
				<TR>
					...
					<tr>
						...
						<td ><p class="LIPoint"><B><span class="font_orange">상품리뷰 보기창 매크로명령어</span></B>(해당 매크로명령어는 다른 페이지 디자인 작업시 사용이 불가능함)</p></td>
					</tr>
					<tr>
