...
if($type=="update" && strlen($body)>0) {
	...
	if($row->cnt==0) {
		...
		$sql.= "type		= 'surveylist', ";
		$sql.= "subject		= '투표리스트 화면 디자인', ";
		$sql.= "body		= '".$body."' ";
		mysql_query($sql,get_db_conn());
	} else {
		$sql = "UPDATE tbldesignnewpage SET ";
		$sql.= "body		= '".$body."' ";
		$sql.= "WHERE type='surveylist' ";
		mysql_query($sql,get_db_conn());
	}
	mysql_free_result($result);
	$onload="<script>alert(\"투표리스트 화면 디자인 수정이 완료되었습니다.\");</script>";
} else if($type=="delete") {
	$sql = "DELETE FROM tbldesignnewpage WHERE type='surveylist' ";
	mysql_query($sql,get_db_conn());
	$onload="<script>alert(\"투표리스트 화면 디자인 삭제가 완료되었습니다.\");</script>";
} else if($type=="clear") {
	$body="";
...
//매크로 보기(팝업)
function macroview(){
	window.open("http://www.getmall.co.kr/macro/pages/surverylist_macro.html","surverylist_macro","height=800,width=680,scrollbars=no");
}
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td height="29" colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td height="28" class="link" align="left" background="images/con_link_bg.gif"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 : 디자인관리 &gt; 개별디자인-페이지 본문 &gt; <span class="2depth_select">투표리스트 화면 꾸미기</span></td>
			</tr>
			</table>
		</td>
	</tr>
    ...
    <tr>
        ...
        <td bgcolor="#ffffff" style="padding:10px">
			...
			<tr>
				...
				<TR>
					<TD><IMG SRC="images/design_eachsurveylist_title.gif" ALT=""></TD>
					</tr>
<tr>
<TD width="100%" background="images/title_bg.gif" height="21"></TD>
			...
			<tr>
				...
				<TR>
					...
					<TD width="100%" class="notice_blue">투표리스트 화면 디자인을 자유롭게 디자인 하실 수 있습니다.</TD>
			...
			<tr>
				...
				<TABLE WIDTH="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0>
				<TR>
					<TD><IMG SRC="images/design_eachsurvey_stitle1.gif" WIDTH="250" HEIGHT=31 ALT=""></TD>
					<TD width="100%" background="images/shop_basicinfo_stitle_bg.gif">
						&nbsp;&nbsp;<a href="javascript:macroview();"><img src="images/btn_macroview.gif" border="0" align="absmiddle" alt="" /></a>&nbsp;&nbsp;<a href="http://www.getmall.co.kr/macro/data/surveryview_design.zip"><img src="images/btn_defaultcode.gif" border="0" align="absmiddle" alt="" />
					</TD>
			...
			<tr>
				...
				<TR>
					<TD background="images/distribute_04.gif"></TD>
					<TD class="notice_blue"><IMG SRC="images/distribute_img.gif" ></TD>
					<TD width="100%" class="notice_blue">1) 매뉴얼의 <b>매크로명령어</b>를 참조하여 디자인 하세요.</span><br>2) [기본값복원]+[적용하기], [삭제하기]하면 기본디자인으로 변경됩니다.</TD>
					<TD background="images/distribute_07.gif"></TD>
				</TR>
				<TR>
					<TD><IMG SRC="images/distribute_08.gif"></TD>
...
function prevPage(){
	...
	window.open("","surveylist","width=510,height=530,scrollbars=no,resizeable=no");

	f = document.prevForm;
	f.mode.value = 'surveylist';
	f.code.value = document.form1.body.value;
	f.submit();
}
</script>

<form name="prevForm" method="post" action="design_prev_post.php" target="surveylist">
