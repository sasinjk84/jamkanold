...
$sql ="SELECT cnt,MID(date,9,2) as hour FROM tblcounterorder ";
$sql.="WHERE (date>='".$prevdate."' AND date<='".$date."') ";
...
for($i=0;$i<10;$i++){
	...
	if($time[$curdate]>0){
		...
		else if($i<>9) ImageCopyResized($im,$im2,$x-2,$y,0,0,7,$height,7,1);
