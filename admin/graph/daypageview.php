...
$im=ImagecreateFromGif("img/daypageview_graph.gif");
...
if($date>=date("Ym",mktime(0,0,0,date("m")-1,date("d"),date("Y")))) {
	...
	$sql ="SELECT MID(date,7,2) as day, pagecnt FROM tblcountermonth WHERE date LIKE '".$date."%'";
}
$result = mysql_query($sql,get_db_conn());
while($row = mysql_fetch_object($result)){
	$time[$row->day]=$row->pagecnt;
	if($topvalue<$row->pagecnt) $topvalue=$row->pagecnt;
}
