...
function ProductSend(mode,prcode) {
	if(mode=="modify") {
		...
		parent.loadingIMG.style.display='block';
	}
}
...
<tr>
	<td width="100%" height="100%" valign="top" style="BORDER:#FF8730 2px solid;padding-left:5px;padding-right:5px;" bgcolor="#ffffff">
	<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td width="100%" style="padding-top:2pt; padding-bottom:2pt;" height="30"><B><span class="font_orange">* 정렬방법 :</span></B> <A HREF="javascript:GoSort('date');">진열순</a> | <A HREF="javascript:GoSort('productname');">상품명순</a> | <A HREF="javascript:GoSort('price');">가격순</a> | <A HREF="javascript:GoSort('reservation');">예약상품출하일순</a></td>
	</tr>
	<tr>
		<td width="100%">
...
<?
			if(strlen($code)==12) {
				...
				if($row && ereg("X",$row->type)){
					...
					$sql0.= "FROM tblproduct AS a LEFT OUTER JOIN tblcategorycode AS b ON a.productcode=b.productcode WHERE a.productcode is not null ";
					$sql0.= $qry;				
					/*
					$sql0 = "SELECT COUNT(*) as t_count ";
					$sql0.= "FROM tblproduct AS a WHERE 1=1 ";
					$sql0.= $qry;
				*/
				//	echo $sql0;
...
/*
					...
					$sql.= "a.buyprice,a.quantity,a.reserve,a.reservetype,a.addcode,a.display,a.vender,a.tinyimage,a.selfcode,a.assembleuse, a.reservation,a.rental,rnt.istrust ";
					$sql.= "FROM tblproduct AS a LEFT OUTER JOIN tblcategorycode AS b ON a.productcode=b.productcode left join rent_product rnt on rnt.pridx=a.pridx WHERE 1=1 ";
					*/
					$sql = "SELECT a.option_price, a.productcode,a.productname,a.production,a.sellprice,a.consumerprice, ";
					$sql.= "a.buyprice,a.quantity,a.reserve,a.reservetype,a.addcode,a.display,a.vender,a.tinyimage,a.selfcode,a.assembleuse, a.reservation,a.rental,a.pridx,rnt.istrust,if(req.productcode is not null,count(req.productcode),NULL) as reqcnt ";
					$sql.= "FROM tblproduct a LEFT OUTER JOIN tblcategorycode AS b ON a.productcode=b.productcode left join rent_product rnt on rnt.pridx=a.pridx left join discount_chgrequest req on req.productcode=a.productcode WHERE a.productcode is not null ";
					$sql.=$qry;
					...
					if($t_count > 0 && mysql_num_rows($result)){
						while($row=mysql_fetch_object($result)) {
							...
							if( $row->reservation != "0000-00-00" ) {
								$reservationView = "<font color='ff0000'><strong>[예약 판매 상품 : ".$row->reservation."]</strong></font><br />";
							}
	*/
							echo "<tr>\n";
							echo "	<TD colspan=\"".$colspan."\" style=\"height:1px;\" background=\"images/table_con_line.gif\"></TD>\n";
							echo "</tr>\n";
							...
							echo "		</div>\n";
							echo "	</td>\n";
							echo "	<TD class=\"td_con1\" align=\"left\" style=\"word-break:break-all;\">";
							if($row->reqcnt > 0){
								echo '<span style="color:red">[변경요청]</span>';
							}
							if($row->rental == '2'){
								echo '[렌탈]';
								if($row->vender == '0') echo '본사';
								else{
									switch($row->istrust){
										case '1': echo '셀프';break;
										case '-1': echo '위탁-승인대기';break;
										case '0': echo '위탁';break;
										default: echo '오류';break;
									}
								}
								echo '<br>';
							}
							echo $reservationView."<img src=\"images/producttype".($row->assembleuse=="Y"?"y":"n").".gif\" border=\"0\" align=\"absmiddle\" hspace=\"2\"><A HREF=\"javascript:ProductSend('modify','".$row->productcode."');\">".$row->productname.($row->selfcode?"-".$row->selfcode:"").($row->addcode?"-".$row->addcode:"")."</A>&nbsp;</td>\n";
