...
if ($type=="up") {
	...
	if ($up_flash[name]) {
		if (strtolower(substr($up_flash[name],strlen($up_flash[name])-3,3))!="swf") {
			...
			$onload = "<script>alert (\"올리실 플래쉬 파일의 용량은 2MB 이하의 swf파일만 가능합니다.\");</script>";
		} else {
        ...
        <td bgcolor="#ffffff" style="padding:10px">
			...
			<tr>
				...
				<tr>
					...
					<td class="td_con1"><input type=file name="up_flash" class="input" <? if (file_exists($imagepath."social_logo.swf")) { ?> style="width:82%"> <input type=button value="이미지삭제" onClick="CheckForm('flash');" class="submit1"><?}else{?> style="width:500px"><?}?><br>
					* 등록 가능한 플래시는 파일 확장자 <span class="font_orange2">SWF(swf)</span> 만 가능하며 용량은 <span class="font_orange2">최대 2MB</span> 까지 가능합니다.<br>* 플래시 등록 후 디자인 입력란에 <span class="font_orange2">매크로명령어 [MAINFLASH_가로X세로]</span>를 입력하면 화면에 출력됩니다.</td>
				</tr>
				<tr>
					<td height="1" colspan="2" bgcolor="#EDEDED"></td>
				</tr>
