...
$max=50;
$specialname=array("","메인 신상품","메인 인기상품","메인 추천상품","메인 특별상품");
...
if ($mode=="sequence" && $change=="Y" && strlen($selcodes)>0 && strlen($special)>0) {
	$sql = "UPDATE tblspecialmain SET special_list = '".$selcodes."' WHERE special='".$special."' ";
	mysql_query($sql,get_db_conn());
	$onload="<script>alert('메인화면 상품 순서 조정이 완료되었습니다.');</script>\n";
} else if ($mode=="modify" && strlen($selcodes)>0 && strlen($special)>0) {
	$sql = "UPDATE tblspecialmain SET special_list = '".$selcodes."' WHERE special='".$special."' ";
	mysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 메인화면 진열상품 카테고리에 추가하였습니다.');</script>\n";
} else if ($mode=="insert" && strlen($selcodes)>0 && strlen($special)>0) {
	$sql = "INSERT tblspecialmain SET ";
	$sql.= "special			= '".$special."', ";
	$sql.= "special_list	= '".$selcodes."' ";
	mysql_query($sql,get_db_conn());
	$onload="<script>alert('해당 상품을 메인화면 진열상품 카테고리에 추가하였습니다.');</script>\n";
} else if ($mode=="delete" && strlen($special)>0) {
	if(strlen($selcodes)==0) {
		$sql = "DELETE FROM tblspecialmain WHERE special='".$special."' ";
	} else {
		$sql = "UPDATE tblspecialmain SET special_list = '".$selcodes."' WHERE special='".$special."' ";
	}
	mysql_query($sql,get_db_conn());
...
function Delete(delcode) {
	ProductInfoStop = "1";
	if(!confirm("해당 상품을 메인상품에서 삭제하시겠습니까?")) return;
	val="";
	for(i=0;i<all_list.length;i++)
	{
