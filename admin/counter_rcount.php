...
$ip = $_POST['ip'];
if($ip) {
	$sql2 = "SELECT count(*) as cnt FROM tbldenyip WHERE ip='{$ip}'";
	$result2 = mysql_query($sql2,get_db_conn());
	$row2 = mysql_fetch_array($result2);
	mysql_free_result($result2);
	
	if($row2['cnt']==0) {
		$sql = "DELETE FROM tblrealcount WHERE ip='{$ip}'";
		mysql_query($sql,get_db_conn());

		$signdate = time();
		$sql = "INSERT INTO tbldenyip VALUES('','{$ip}','{$signdate}')";
		mysql_query($sql,get_db_conn());
	}
}
...
<tr>
	...
	<tr>
		...
		<tr>
			<td height="28" class="link" align="right"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 :통계분석 &gt; <span class="2depth_select">실시간 접속자 현황</span></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<tr>
				...
				<TR>
					...
					<TD width="100%" class="notice_blue">실시간 접속자 현황을 보실 수 있습니다.</TD>
			...
			<tr>
				...
				<TR align=center>
					...
					<TD class="table_cell1">접속IP</TD>
					<TD class="table_cell1">체류시간</TD>
					<TD class="table_cell1">접속횟수</TD>
				</TR>
				<TR>
					<TD colspan="5" background="images/table_con_line.gif"></TD>
				</TR>
<?
				$sql = "SELECT * FROM tblrealcount ";
				$sql.= "WHERE uid>0 ";

				if($search) {
					$sql .= " && (id like '%{$search}%' || ip like '%{$search}%') ";
				}
				...
				while($row=mysql_fetch_object($result)) {
					...
					$acc_time = time() - $row->acc_date;
					if($acc_time>60) {
						$acc_time = number_format($acc_time/60)."분".number_format($acc_time%60)."초";
					}
					else $acc_time = $acc_time."초";
					...
					if($row->id==$row->ip) $row->id = '';
					
					echo "<tr>";
					echo "	<TD align=center class=\"td_con2\">".$number."</td>\n";
					echo "	<TD align=center class=\"td_con1\">";
					if($row->id) {
						echo "<FONT class=mainprname>".$row->id."</font> <A HREF=\"javascript:MemberView('".$row->id."');\"><img src=\"images/bnt_memberview.gif\" width=\"74\" height=\"16\" border=\"0\" align='absmiddle'></A>";
					}
					else echo ".";
					echo "</td>\n";
					echo "	<TD align=center class=\"td_con1\">{$row->ip} <font onclick='denyIp(\"{$row->ip}\");' style='cursor:pointer'>[IP차단하기]</font></td>\n";
					echo "	<TD align=center class=\"td_con1\">{$acc_time}</td>\n";
					echo "	<TD align=center class=\"td_con1\">".number_format($cnts)."회</td>\n";
					echo "</tr>\n";
					echo "<tr>\n";
