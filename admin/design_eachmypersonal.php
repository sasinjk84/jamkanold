...
$subject = '마이페이지 1:1고객문의';

$insertKey = 'mypersonal';

// 백업 / 복구
if ( $type=="store" OR $type=="restore" ) {
	$MSG = adminDesingBackup ( $type, $insertKey, $body, $subject, '', '', $leftmenu );
	$MSG = adminDesingBackup ( $type, 'design_mypersonal', 'U', $subject, '', '', '', 'tblshopinfo', 'design_mypersonal' );
...
if($type=="update" && strlen($body)>0) {
	...
	$sql = "SELECT COUNT(*) as cnt FROM tbldesignnewpage WHERE type='mypersonal' ";
	$result=mysql_query($sql,get_db_conn());
	$row=mysql_fetch_object($result);
	if($row->cnt==0) {
		$sql = "INSERT tbldesignnewpage SET ";
		$sql.= "type		= 'mypersonal', ";
		$sql.= "subject		= '마이페이지 1:1고객문의 화면', ";
		...
		$sql.= "leftmenu	= '".$leftmenu."', ";
		$sql.= "body		= '".$body."' ";
		$sql.= "WHERE type='mypersonal' ";
		mysql_query($sql,get_db_conn());
	}
	mysql_free_result($result);

	$sql = "UPDATE tblshopinfo SET design_mypersonal='U' ";
	mysql_query($sql,get_db_conn());
	DeleteCache("tblshopinfo.cache");
	$onload="<script>alert(\"마이페이지 1:1고객문의 화면 디자인 수정이 완료되었습니다.\");</script>";
} else if($type=="delete") {
	$sql = "DELETE FROM tbldesignnewpage WHERE type='mypersonal' ";
	mysql_query($sql,get_db_conn());

	$sql = "UPDATE tblshopinfo SET design_mypersonal='001' ";
	mysql_query($sql,get_db_conn());
	DeleteCache("tblshopinfo.cache");
	$onload="<script>alert(\"마이페이지 1:1고객문의 화면 디자인 삭제가 완료되었습니다.\");</script>";
} else if($type=="clear") {
	$intitle="";
	$body="";
	$sql = "SELECT body FROM tbldesigndefault WHERE type='mypersonal' ";
	$result=mysql_query($sql,get_db_conn());
...
if($type!="clear") {
	$body="";
	$intitle="";
	$sql = "SELECT leftmenu,body FROM tbldesignnewpage WHERE type='mypersonal' ";
	$result = mysql_query($sql,get_db_conn());
	if($row=mysql_fetch_object($result)) {
		$body=$row->body;
...
function CheckForm(type) {
	if(type=="update") {
		if(document.form1.body.value.length==0) {
			alert("마이페이지 1:1고객문의 화면 디자인 내용을 입력하세요.");
		...
		if(confirm("마이페이지 1:1고객문의 화면 디자인을 삭제하시겠습니까?")) {
			document.form1.type.value=type;
			document.form1.submit();
		}
	} else if(type=="clear") {
...
//매크로 보기(팝업)
function macroview(){
	window.open("http://www.getmall.co.kr/macro/pages/mtom_macro.html","mtom_macro","height=800,width=680,scrollbars=no");
}
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td height="29" colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td height="28" class="link" align="left" background="images/con_link_bg.gif"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 : 디자인관리 &gt; 개별디자인-페이지 본문 &gt; <span class="2depth_select">1:1고객문의 화면 꾸미기</span></td>
			</tr>
			</table>
		</td>
	</tr>
    ...
    <tr>
        ...
        <td bgcolor="#ffffff" style="padding:10px">
			...
			<tr>
				<td>
					<TABLE WIDTH="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0>
						<TR>
							<TD><IMG SRC="images/design_eachmypersonal_title.gif"  ALT=""></TD>
						</tr>
						<tr>
				...
				<td style="padding-bottom:3pt;">
					<TABLE WIDTH="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0>
						...
						<TR>
							...
							<TD width="100%" class="notice_blue">마이페이지 1:1고객문의 화면 디자인을 자유롭게 디자인 하실 수 있습니다.</TD>
			...
			<tr>
				...
				<TABLE WIDTH="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0>
				<TR>
					<TD><IMG SRC="images/design_eachmypersonal_stitl.gif"  ALT=""></TD>
					<TD width="100%" background="images/shop_basicinfo_stitle_bg.gif">
						&nbsp;&nbsp;<a href="javascript:macroview();"><img src="images/btn_macroview.gif" border="0" align="absmiddle" alt="" /></a>&nbsp;&nbsp;<a href="http://www.getmall.co.kr/macro/data/mtom_design.zip"><img src="images/btn_defaultcode.gif" border="0" align="absmiddle" alt="" />
					</TD>
			...
			<tr>
				...
				<TR>
					...
					<tr>
						...
						<td ><p class="LIPoint"><B><span class="font_orange">1:1고객문의 화면 매크로명령어</span></B>(해당 매크로명령어는 다른 페이지 디자인 작업시 사용이 불가능함)</p></td>
					</tr>
					<tr>
						...
						<tr>
							...
							<td class=td_con1 style="padding-left:5;">
							총 문의수 <FONT class=font_blue>(예:총 [TOTAL]건의 문의내역이 있습니다.)</font>
							</td>
						</tr>
						<tr><td colspan=2 height=1 bgcolor=#dddddd></td></tr>
						<tr>
							<td class=table_cell align=right style="padding-right:15">[WRITE]</td>
							<td class=td_con1 style="padding-left:5;">
							질문하기 버튼 <FONT class=font_blue>(예:&lt;a href=[WRITE]>질문하기&lt;/a>)</font>
							</td>
						</tr>
						<tr><td colspan=2 height=1 bgcolor=#dddddd></td></tr>
						<tr>
...
<FONT class=font_blue>   [IFPERSONAL]
   ...
   [IFENDPERSONAL]</font></pre>
						...
						<tr><td colspan=2 height=1 bgcolor=#dddddd></td></tr>
						<tr>
							<td class=table_cell align=right style="padding-right:15">[PERSONAL_NUM]</td>
							<td class=td_con1 style="padding-left:5;">
							번호
							</td>
						</tr>
