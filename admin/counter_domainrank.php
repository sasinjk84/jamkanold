...
$len=30;
if($type=="d") {
	$sql ="SELECT * FROM tblcounterdomain WHERE date='".$searchdate."' ORDER BY cnt DESC LIMIT ".$len;
} else if($type=="w") {
	...
	$sql.="GROUP BY domain ORDER BY cnt DESC LIMIT ".$len;
} else if($type=="m") {
	$date=substr($searchdate,0,6);  
	if ($date==date("Ym")) {
		$sql ="SELECT SUM(cnt) as cnt,domain FROM tblcounterdomain ";
		$sql.="WHERE date LIKE '".$date."%' GROUP BY domain ORDER BY cnt DESC LIMIT ".$len;
	} else {
		$sql ="SELECT cnt,domain FROM tblcounterdomainmonth ";
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td height="29" colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td height="28" class="link" align="left" background="images/con_link_bg.gif"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 : 통계분석 &gt; 외부 접근 경로 분석 &gt; <span class="2depth_select">도메인별 접근순위</span></td>
			</tr>
			</table>
		</td>
	</tr>   
...
<?}?>
			<tr>
				...
				<TR>
					<TD><IMG SRC="images/counter_domainrank_title.gif" ALT=""></TD>
					</tr>
<tr>
...
?>
				...
				<tr>
					...
					<TR>
						...
						<TD class="table_cell" align=center><FONT color=#3d3d3d>도메인명</FONT></TD>
...
<?
					...
					for($i=0;$i<$len;$i++){
						...
						echo "	<TD class=\"td_con2a\" style=\"padding-left:5\"><A HREF=\"http://".$domain[$i]."\" target=\"_blank\">http://".$domain[$i]."</A></td>\n";
			...
			<tr>
				...
				<TR>
					...
					<tr>
						...
						<td><FONT color=#3d3d3d>당사 쇼핑몰을 외부에서 찾아 오는 경로(도메인)를 순위별로 알 수 있습니다.</FONT></td>
					</tr>
					<tr><td colspan=2 height=5></td></tr>
					<tr>
						<td width="20" align="right" valign="top"><img src="images/icon_8.gif" width="13" height="18" border="0"></td>
						<td>
							<FONT color=#3d3d3d>외부에서 당사 쇼핑몰을 찾아오는 경로를 분석하여, 보다 효과적으로 방문자 숫자를 증가시킬 수 있습니다.</FONT>
						</td>
					</tr>
					<tr><td colspan=2 height=5></td></tr>
					<tr>
						<td width="20" align="right" valign="top"><img src="images/icon_8.gif" width="13" height="18" border="0"></td>
						<td>
							<FONT color=#3d3d3d>외부에서 당사 쇼핑몰을 찾아오는 경로(도메인)를 파악하여, 그 사이트와 전략적인 제휴 등 적극적인 마케팅을 전개할 수 있습니다.</FONT>
						</td>
					</tr>
					</table>
					</TD>
