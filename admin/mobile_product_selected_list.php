<?
	...
	$search_world = $_REQUEST["search_word"];
...
<script language="JavaScript">
	...
	function selDel(idx) {
		...
		if(confirm("선택하신 상품을 모바일샵 상품목록 보이기에서 삭제 하시겠습니까?\n(실제 상품데이터가 삭제되지는 않습니다)")){
			ifrm_ctrl.location.href="mobile_product_ctrl.php?mode=mobile_display_n&productcode="+idx;
		}
	...
	function openSel(page_mode,code) {
		window.open("mobile_product_search.php?page_mode="+page_mode+"&code="+code,"collection","width=900,height=800,scrollbars=yes");
	}
			...
			<table cellpadding="0" cellspacing="0" width="100%">
				<tr>
					<td width="100%" style="padding-top:2pt; padding-bottom:2pt;" height="30">
						<b><span class="font_orange">* 정렬방법 :</span></b>
						<a href="javascript:GoSort('date');">진열순</a> | <a href="javascript:GoSort('productname');">상품명순</a> | <a href="javascript:GoSort('price');">가격순</a>
					</td>
				</tr>
				<tr>
					<td width="100%">
								...
								<tr>
											...
											<?}?>
												...
												<?
													if(strlen($code)==12) {
														...
														$qry = "WHERE b.categorycode LIKE '".$likecode."%' AND mobile_display = 'Y'";
														if(strlen($keyword)>2) {
															$qry.= "AND a.productname LIKE '%".$keyword."%' ";
														}

														$sql0 = "SELECT COUNT(*) as t_count FROM tblproduct AS a LEFT JOIN tblcategorycode AS b ON a.productcode = b.productcode ";
														$sql0.= $qry;
														...
														$sql.= "FROM tblproduct AS a LEFT JOIN tblcategorycode AS b ON a.productcode = b.productcode ";
														$sql.= $qry." ";
														...
														while($row=mysql_fetch_object($result)) {
															...
															echo "	<td class=\"td_con1\" align=center><a href=\"#\"onclick=\"selDel('".$row->productcode."')\"><img src=\"images//btn_del.gif\" border=\"0\"></a></td>\n";
															echo "</tr>\n";
														}
														...
														if ($cnt==0) {
															$page_numberic_type="";
															echo "<tr><td colspan=\"".$colspan."\" background=\"images/table_con_line.gif\"></td></tr><tr><td class=\"td_con2\" colspan=\"".$colspan."\" align=\"center\">'모바일샵에 보이기'로  설정된 상품이 없습니다.</td></tr>";
														}
													} else {
														$page_numberic_type="";
														echo "<tr><td colspan=\"".$colspan."\" background=\"images/table_con_line.gif\"></td></tr><tr><td class=\"td_con2\" colspan=\"".$colspan."\" align=\"center\">선택된 카테고리가 없습니다. 카테고리를 먼저 선택해 주세요.</td></tr>";
													}
												?>
												<tr>
									...
									<?
										if($page_numberic_type) {
											...
											echo "<tr>\n";
											echo "	<td height=\"30\" align=center background=\"images/blueline_bg.gif\">\n";
									...
									<tr>
										...
										<? if($code=="") {?>
											<input type=image  src="images/mobile_product_list_btn.gif" value="추가 등록하기" onClick="alert('카테고리를 선택해 주세요.')">
										<? } else { ?>
											<input type=image src="images/mobile_product_list_btn.gif" value="추가 등록하기" onClick="openSel('category','<?=$code?>')">
										<? } ?>
										</td>
									</tr>
								</table>
