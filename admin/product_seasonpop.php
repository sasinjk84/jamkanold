...
extract($_REQUEST);
if($_REQUEST['act'] == 'add'){	
	$sql = "select count(*) from season_range where code='".$_REQUEST['code']."' and ((start between '".$seasonStartDate."' and '".$seasonEndDate."' and  end between '".$seasonStartDate."' and '".$seasonEndDate."') ||  (start <= '".$seasonStartDate."' and end >= '".$seasonEndDate."'))";
	...
	$insertSQL = "INSERT `season_range` SET `code`='".$code."', `type` = '".$seasonType."', `start` = '".$seasonStartDate."', `end` = '".$seasonEndDate."' ";
	if(false === mysql_query($insertSQL,get_db_conn())){
		_alert('DB 질의 오류','-1');
	}else{
		_alert('등록되었습니다.',$_SERVER['PHP_SELF'].'?code='.$_REQUEST['code']);
	}
	exit;
}else if($_POST['act'] == 'delete' && _isInt($_POST['delidx'])){
	$sql = "delete from season_range where idx='".$_POST['delidx']."' limit 1";
	if(false === mysql_query($sql,get_db_conn())){
		_alert('삭제','-1');
	}else{
		_alert('삭제되었습니다.',$_SERVER['PHP_SELF'].'?code='.$_REQUEST['code']);
	}
	exit;
}else{
	$sql = "select * from season_range where code='".$_REQUEST['code']."' order by start desc";
	if(false === $res = mysql_query($sql,get_db_conn())){
		_alert('DB 질의 오류',0);
		exit;
	}
...
<body>
	...
	<h2 style="background:url('images/member_mailallsend_imgbg.gif');"><img src="images/season_popt.gif" alt="성수기/준성수기 등록" /></h2>

	<TABLE WIDTH="96%" align="center" BORDER=0 CELLPADDING=0 CELLSPACING=0>
		<TR>
			<TD><IMG SRC="images/product_season_stitle3.gif" ALT="성수기/준성수기 등록" /></TD>
		</TR>
	</TABLE>
	<TABLE WIDTH="96%" align="center" BORDER=0 CELLPADDING=0 CELLSPACING=0>
		<TR>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="96%" align="center" style="margin-bottom:15px; border-bottom:1px solid #ccc; border-top:1px solid #ccc">
		<tr>
			<td style="width:120px;" class="table_cell"><img width="8" height="11" src="images/icon_point2.gif" border="0"/>등록</td>
			<td class="td_con1">
			...
			<td style="width:140px;" class="table_cell"><img width="8" height="11" src="images/icon_point2.gif" border="0"/> 시작일~종료일</td>
			<td class="td_con1">
