...
if ( $type=="store" OR $type=="restore" ) {
	...
	$MSG .= adminDesingBackup ( $type, 'design_mbjoin', 'U', $subject, '', '', '', 'tblshopinfo', 'design_mbjoin' );
...
if($type=="update" && strlen($body)>0) {
	...
	$sql = "SELECT COUNT(*) as cnt FROM tbldesignnewpage WHERE type='mbjoin' ";
	$result=mysql_query($sql,get_db_conn());
	$row=mysql_fetch_object($result);
	if($row->cnt==0) {
		$sql = "INSERT tbldesignnewpage SET ";
		$sql.= "type		= 'mbjoin', ";
		$sql.= "subject		= '회원가입 입력폼 디자인', ";
		...
		$sql.= "leftmenu	= '".$leftmenu."', ";
		$sql.= "body		= '".$body."' ";
		$sql.= "WHERE type='mbjoin' ";
		mysql_query($sql,get_db_conn());
	}
	mysql_free_result($result);

	$sql = "UPDATE tblshopinfo SET design_mbjoin='U' ";
	mysql_query($sql,get_db_conn());
	DeleteCache("tblshopinfo.cache");
	$onload="<script>alert(\"회원가입 입력폼 디자인 수정이 완료되었습니다.\");</script>";
} else if($type=="delete") {
	$sql = "DELETE FROM tbldesignnewpage WHERE type='mbjoin' ";
	mysql_query($sql,get_db_conn());

	$sql = "UPDATE tblshopinfo SET design_mbjoin='001' ";
	mysql_query($sql,get_db_conn());
	DeleteCache("tblshopinfo.cache");
	$onload="<script>alert(\"회원가입 입력폼 디자인 삭제가 완료되었습니다.\");</script>";
} else if($type=="clear") {
	$intitle="";
	$body="";
	$sql = "SELECT body FROM tbldesigndefault WHERE type='mbjoin' ";
	$result=mysql_query($sql,get_db_conn());
...
if($type!="clear") {
	$body="";
	$intitle="";
	$sql = "SELECT leftmenu,body FROM tbldesignnewpage WHERE type='mbjoin' ";
	$result = mysql_query($sql,get_db_conn());
	if($row=mysql_fetch_object($result)) {
		$body=$row->body;
...
function CheckForm(type) {
	if(type=="update") {
		...
		if(confirm("회원가입 입력폼 디자인을 삭제하시겠습니까?")) {
			document.form1.type.value=type;
			document.form1.submit();
		}
	} else if(type=="clear") {
...
//매크로 보기(팝업)
function macroview(){
	window.open("http://www.getmall.co.kr/macro/pages/memjoin_macro.html","memjoin_macro","height=800,width=680,scrollbars=no");
}
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td height="29" colspan="3">
			<table cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td height="28" class="link" align="left" background="images/con_link_bg.gif"><img src="images/top_link_house.gif" border="0" valign="absmiddle">현재위치 : 디자인관리 &gt; 개별디자인-페이지 본문 &gt; <span class="2depth_select">회원가입 입력폼 꾸미기</span></td>
			</tr>
			</table>
		</td>
	</tr>
    ...
    <tr>
        ...
        <td bgcolor="#ffffff" style="padding:10px">
			...
			<tr>
				...
				<TR>
					<TD><IMG SRC="images/design_eachmember_title.gif" ALT=""></TD>
					</tr>
<tr>
<TD width="100%" background="images/title_bg.gif" height="21"></TD>
			...
			<tr>
				...
				<TR>
					...
					<TD width="100%" class="notice_blue"><p>쇼핑몰 회원가입 입력폼 디자인을 자유롭게 디자인 하실 수 있습니다.</p></TD>
			...
			<tr>
				...
				<TABLE WIDTH="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0>
				<TR>
					<TD><IMG SRC="images/design_eachmemb_stitle1.gif" WIDTH="250" HEIGHT=31 ALT=""></TD>
					<TD width="100%" background="images/shop_basicinfo_stitle_bg.gif">
						&nbsp;&nbsp;<a href="javascript:macroview();"><img src="images/btn_macroview.gif" border="0" align="absmiddle" alt="" /></a>&nbsp;&nbsp;<a href="http://www.getmall.co.kr/macro/data/memjoin_design.zip"><img src="images/btn_defaultcode.gif" border="0" align="absmiddle" alt="" />
					</TD>
			...
			<tr>
				...
				<TR>
					...
					<tr>
						...
						<td ><p class="LIPoint"><B><span class="font_orange">회원가입 매크로명령어</span></B>(해당 매크로명령어는 다른 페이지 디자인 작업시 사용이 불가능함)</p></td>
					</tr>
					<tr>
						...
						<tr><td colspan=2 height=1 bgcolor=#dddddd></td></tr>
						<tr>
							<td class=table_cell align=right style="padding-right:15">[IDCHECK]</td>
							<td class=td_con1 style="padding-left:5;">
							회원ID 중복확인 <FONT class=font_blue>(예:&lt;a href=[IDCHECK]>중복확인&lt;/a>)</font>
							</td>
						</tr>
						<tr><td colspan=2 height=1 bgcolor=#dddddd></td></tr>
						<tr>
