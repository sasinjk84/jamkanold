...
if($_REQUEST['act'] == 'update'){	
	foreach($_REQUEST['gdiscount'] as $groupcode=>$discount){
		...
		$sql = "insert into reseller_reserve_chgrequest (group_code,productcode,reserve) values ('".$groupcode."','".$_REQUEST['code']."','".($discount/100)."') ON DUPLICATE KEY UPDATE reserve=values(reserve)";		
...
<body style="padding:0px; margin:0px;">
	...
	<h2 style="background:url('/admin/images/member_mailallsend_imgbg.gif'); font-size:15px; color:white; padding:5px 0px 5px 5px;">추천인 적립금 관리</h2>	
	...
	<? 
		if(strlen($code) > 12) $cinfo = getGroupReseller_Reserves(substr($code,0,12));	
		$groupdiscount = getGroupReseller_Reserves($code);	
