...
if($_REQUEST['act'] == 'update'){	
	foreach($_REQUEST['gdiscount'] as $groupcode=>$discount){
		...
		$sql = "insert into tblmemberdiscount (group_code,productcode,discount) values ('".$groupcode."','".$_REQUEST['code']."','".($discount/100)."') ON DUPLICATE KEY UPDATE discount=values(discount)";		
...
<body style="padding:0px; margin:0px;">
	...
	<? 
		if(strlen($code) > 12) $cinfo = getGroupDiscounts(substr($code,0,12));	
		...
		<tr>
			...
			<td  style="padding:5px;" class="norbl"><input type="text" name="gdiscount[<?=$discount['group_code']?>]" value="<?=$discount['discount']*100?>" style="width:40px;" />% <? if(!_empty($cinfo[$discount['group_code']])) echo '('.($cinfo[$discount['group_code']]['discount']*100).'%)';?></td>
		</tr>
	<?			}
		} ?>
	</table>
