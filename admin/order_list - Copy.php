...
<?
							$ardg=array("\"\":전체선택","S:발송준비","Y:배송","N:미처리","C:주문취소","YA:교환반품신청","YB:교환반품접수","YC:교환반품완료","R:반송","D:취소요청","E:환불대기","H:배송(정산보류)");
							for($i=0;$i<count($ardg);$i++) {
								$tmp=split(":",$ardg[$i]);
								echo "<option value=\"".$tmp[0]."\" ";
...
?>
						...
						<tr>
							<TD class="table_cell" width="139"><img src="images/icon_point5.gif" width="8" height="11" border="0">주문타입</TD>
							<TD class="td_con1" width="613"><select name="s_type" class="select">
							<option value="" <?if(strlen($s_type)==0)echo"selected";?>>전체선택</option>
							<option value="0" <?if($s_type=="0")echo"selected";?>>일반주문</option>
							<option value="3" <?if($s_type=="3")echo"selected";?>>소셜쇼핑</option>
							<option value="1" <?if($s_type=="1")echo"selected";?>>상품권(선물하기)</option>
							<option value="2" <?if($s_type=="2")echo"selected";?>>상품권(본인구매)</option>
							</select></TD>
						</tr>
						<TR>
							<TD colspan="2" background="images/table_con_line.gif"></TD>
			...
			<tr>
				<td>
					...
					<?
						// 예약 상품 리스트
						$listOption['vdate'] = date("Ymdhis"); // 출력 일자
						$listOption['more'] = "A"; // 선택일 이후 모두 표시 (A)
							...
							<td>
								...
								<table border=0 width="100%" cellpadding="0" cellspacing="0">
									...
									<tr>
										...
										<td align="center" class="table_cell1">상품명 ([배송예약일] 예약상품)</td>
				...
				while($row=mysql_fetch_object($result)) {
					...
					while($row2=mysql_fetch_object($result2)) {
						...
						if($vendercnt>0) {
							...
							$prval.=$tinyImg;
							$prval.="	<td class=\"td_con6\" style=\"font-size:8pt;padding:3;line-height:10pt\">".((substr($row2->productcode,0,3) == '899')?'<span style="font-weight:bold;color:orange">[타임세일]</span>':'')."<font color=#0066cc>".titleCut(58,$row2->productname)."</font>";
						} else{
							$prval.=$tinyImg;
							$prval.="	<td class=\"td_con6\" style=\"font-size:8pt;padding:3;line-height:10pt\">".((substr($row2->productcode,0,3) == '899')?'<span style="font-weight:bold;color:orange">[타임세일]</span>':'')."<font color=#0066cc>".titleCut(58,$row2->productname)."</font>";
						}
						...
						if( strlen($row2->bookingStartDate) > 0 AND strlen($row2->bookingEndDate) > 0 ) {
							...
							$prval .= "<font color='0000FF'><strong>[대여상품 : ".$row2->bookingStartDate." ~ ".$row2->bookingEndDate."]</strong></font><br>".$printOptions." ";
						}
