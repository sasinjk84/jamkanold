...
if($_POST['act'] == 'add'){
	...
	if(_array($weekend)){
		...
		_alert('정상 처리 되었습니다.','/admin/product_holiday.php?code='.$indata['code']);
		exit;
	}else{
		...
		if(false === mysql_query($sql,get_db_conn())){
			...
			_alert('등록 되었습니다.',$_SERVER['PHP_SELF'].'?code='.$_POST['code']);
		}
	}
	exit;
	...
	if(false === mysql_query($sql,get_db_conn())){
		...
		_alert('삭제 되었습니다.',$_SERVER['PHP_SELF'].'?code='.$_POST['code']);
	}
	exit;
}
...
<body>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="96%" align="center" style="margin-bottom:15px;">
		...
		<tr>
			<td class="table_cell"><img width="8" height="11" src="images/icon_point2.gif" border="0"/> 주말요금</td>
			<td class="td_con1"><input type="checkbox" name="weekend[]" value="sun" <?=$weekendconf['sun']=='ok'?'checked':''?> />일요일 <input type="checkbox" name="weekend[]" value="sat" <?=$weekendconf['sat'] == 'ok'?'checked':''?> />토요일 <input type="button" value="적용" onClick="submit();"></td>
		</tr>
		<tr><td background="images/table_top_line.gif" colSpan="2" style="height:1px" /></td></tr>
	</table>
	</form>
	<table border="0" cellpadding="0" cellspacing="0" width="96%" align="center" class="tableBase">
		...
		if(count($items) < 1){ ?>
		<tr>
			<td colspan="3" style="text-align:center; padding:5px 0px;">등록된 휴일정보가 없습니다.</td>
		</tr>
<?		}else{ 
		...
		<tr style="text-align:center">
			<td  class="firstTd"><? if($item['code'] == '000000000000') echo '전체'; ?><?=(_empty($item['year'])?'[매년]&nbsp;':$item['year'])?><?=substr($item['date'],0,2).'-'.substr($item['date'],2,2)?></td>
			<td><?=$item['title']?></td>
