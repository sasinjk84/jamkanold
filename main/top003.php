...
<div class="topLogoAndSearchWrap">
	<div class="topLogoAndSearch">
		<div class="topLogo">
			<a href="<?=$Dir.MainDir?>main.php" <?=$main_target?>><!--<span>잠깐 쓸 땐~</span><br />--><?=$logo?></a>
		</div>
...
-->
						<!-- 2018.10.24 
						<div class="searchCalendal"><input type="text" name="bookingsdate" class="datePickInput" id="bookingsdate" value="<?//=date("Ymd")?>" placeholder="대여" readonly /></div>
						<div class="searchCalendal">&nbsp;~&nbsp;</div>
						<div class="searchCalendal"><input type="text" name="bookingedate" class="datePickInput" id="bookingedate" value="<?//=date("Ymd")?>" placeholder="반납" readonly /></div>
						-->
						...
						<div class="searchInput" id="searchInputDiv"><input type="text" name="search" id="searchInputFld" value="<?=$_POST["search"]?>" placeholder="잠깐 빌려쓰고 싶을때, 잠깐닷컴" onKeyDown="CheckKeyTopSearch();" /></div>
		...
		<div class="topMenuRight">
			<ul class="prMenu">
				<li style="border-right:1px solid #ededed;"><a href="/front/community.php?code=1">고객센터</a></li>
				<li><a href="/front/venderProposal.php">입점문의</a></li>
			</ul>
		</div>
	</div>
</div>
...
/*							.categoryAllTBL th{ height:30px; border-bottom:1px solid #ff0000; border-left:1px solid #efefef; text-align:center;width:<?=round(100/count($categoryitems['items']))?>%; color: #ea2f36;}*/
				...
				<? /*
					...
					*/ 
					...
					<!--<li><a href="/front/community.php?code=2">커뮤니티</a>-->
					*/
					?>
				</ul>
				<ul class="topCategoryMenu">
					<li class="cart" onClick="location.href='<?=$Dir.FrontDir?>basket.php'" style="overflow:hidden;">
						<img src="/data/design/img/top/icon_cart.png" align="absmiddle" style="margin-right:3px;vertical-align:-5px" /><span style="padding:0px 4px">카트</span>
					...
					<li class="wishList" onMouseOver="wishlistView('wishProduct','open')" onMouseOut="mypageView('wishProduct','out')">
						<a href="/front/wishlist.php"><img src="/data/design/img/top/icon_wish.png" align="absmiddle" style="margin-right:3px;vertical-align:-5px" />찜</a>
						<div id="wishProduct" onMouseOver="wishlistView('wishProduct','over')" onMouseOut="mypageView('wishProduct','out')">
							...
							if(_empty($_ShopInfo->getMemid())){
								echo "<div style=\"text-align:center;font-size:12px;color:#ea2f36;\">회원 로그인이<br />필요합니다.</div>";
							}else{
								...
								if( $wishCnt > 3 ) {
									echo "<div><a href=\"/front/wishlist.php\"><img src=\"/data/design/img/top/btn_wishlist_next.gif\" border=\"0\" alt=\"더보기\" /></a></div>";
								}
							}
							?>
						</div>
					...
					<li class="signAccount">
						<a href="#" onMouseOver="mypageView('memberMenuAll','open')" onMouseOut="mypageView('memberMenuAll','out')">
							...
							<span style="vertical-align:middle;padding-right:3px;"><img src="/data/design/img/top/icon_member.png"></span>로그인/가입
							<?}else{?>
						...
						<div id="memberMenuAll" onMouseOver="mypageView('memberMenuAll','over')" onMouseOut="mypageView('memberMenuAll','out')">
							<ul>
								<? if(strlen($_ShopInfo->getMemid())==0){ ######## 로그인을 안했다#######?>
									...
									<li class="firstLi"><a href="<?=$Dir.FrontDir?>login.php" >로그인</a></li>
<!--									<li class="firstLi"><a href="<?=$Dir.FrontDir?>login.php" class="button white medium">로그인</a></li>-->
									<li><a href="<?=$Dir.FrontDir?>member_classification.php">회원가입</a></li>
									<li><a href="<?=$Dir.FrontDir?>findpwd.php">ID/PW 찾기</a></li>
									<? if(isWholesale() == 'Y') { ?>
								...
								<? } ?>
								<li><a href="<?=$Dir.FrontDir?>mypage_reserve.php">적립금 관리</a></li>
					...
					*/ ?>
					...
					<? if(strlen($_ShopInfo->getMemid())==0){ ######## 로그인을 안했다#######?>
						<li class="signAccount">
							<a href="<?=$Dir.FrontDir?>login.php" style="display:inline-block;margin-right:20px;"><img src="/data/design/img/top/icon_member.png" align="absmiddle" style="margin-right:3px;vertical-align:-5px" />로그인</a>
							<a href="<?=$Dir.FrontDir?>member_classification.php" style="margin-right:5px">가입</a>
						</li>
					<? }else{ ?>
