...
if (strlen($_data->shop_intro)==0) {
	$mainsec_strI.="<div align=center><img src='../images/002/main_visual_sample.gif'/></div>";
} else {
...
if ($_data->title_type=="Y" && file_exists($Dir.DataDir."design/main_new_title.gif")) {
	...
	$mainsec_strN.="<td style='padding-bottom:20px'>\n";
...
if(count($res)){
	if($main_new_type=="I") {	//이미지A형
		...
		foreach($res as $i=>$row){
			...
			$mainsec_strN.="<tr>\n";
			$mainsec_strN.="	<td align=\"center\" style=\"padding-left:5px;padding-right:5px;word-break:break-all;\" class=\"mainprprice\">".$strikeStart.dickerview($row->etctype,number_format($row->sellprice)."원".$strikeEnd);
		...
		foreach($res as $i=>$row){
			...
			$mainsec_strN.="<br><font class=\"mainprprice\">".$strikeStart.dickerview($row->etctype,number_format($row->sellprice)."원".$strikeEnd)."</font>";
			if ($_data->ETCTYPE["MAINSOLD"]=="Y" && $row->quantity=="0") $mainsec_strN.=soldout();
...
if ($_data->title_type=="Y" && file_exists($Dir.DataDir."design/main_best_title.gif")) {
	...
	$mainsec_strB.="<td width=100% style='padding-bottom:20px'>\n";
...
$mainsec_strB.="	<TR>\n";
$mainsec_strB.="		<TD align=center><img src=".$Dir."images/".$_data->icon_type."/main_skin1_bestitem_bg_head.gif border=0></TD>\n";
...
if(count($res)){
	if($main_best_type=="I") {	//이미지A형
		...
		foreach($res as $i=>$row){
			...
			$mainsec_strB.="<tr>\n";
			$mainsec_strB.="	<td align=\"center\" style=\"padding-left:5px;padding-right:5px;word-break:break-all;\" class=\"mainprprice\">".$strikeStart.dickerview($row->etctype,number_format($row->sellprice)."원".$strikeEnd);
...
if ($_data->title_type=="Y" && file_exists($Dir.DataDir."design/main_hot_title.gif")) {
	...
	$mainsec_strH.="<td width=100% style='padding-bottom:20px'>\n";
...
$mainsec_strH.="	<TR>\n";
$mainsec_strH.="		<TD align=center></TD>\n";
...
while($row=mysql_fetch_object($result)) {
	...
	$mainsec_strG.="			<td valign=top>\n";
	$mainsec_strG.="			<table cellpadding=0 cellspacing=0 width=100%>\n";
	$mainsec_strG.="			<tr><td height=5></td></tr>\n";
	$mainsec_strG.="			<tr>\n";
	...
	if($i%2) {
		$mainsec_strG.="<td align=\"center\"  valign=\"top\" width=\"2%\"><IMG SRC=\"".$Dir."images/".$_data->icon_type."/1_164.gif\" border=\"0\" hspace=\"10\"></td>\n";
	}
}
if($i==1) {
	$mainsec_strG.="<td></td>\n";
...
?>
			...
			<tr>
				<td style="padding-left:5px;padding-right:5px; padding-top:10px">
				<table cellpadding="0" cellspacing="0" width="100%">
				<tr>
					<td>
