...
/* =   01. KCP 지불 서버 정보 설정                                              = */
/* = -------------------------------------------------------------------------- = */
$g_conf_home_dir  = $Dir."paygate/A/payplus/"; // ※ 쇼핑몰 모듈 설치 절대 경로 bin전까지
$g_conf_log_level = "3";
...
$g_conf_site_id   = $tax_scd; // 리얼 반영시 KCP에 발급된 site_cd 사용 ex) T0000
/* ============================================================================== */
...
/* =   01. 요청 정보 설정                                                       = */
/* = -------------------------------------------------------------------------- = */
$req_tx     = "pay";//$_POST[ "req_tx"     ];                             // 요청 종류
$trad_time  = $row->tsdtime;//$_POST[ "trad_time"  ];                             // 원거래 시각
/* = -------------------------------------------------------------------------- = */
$ordr_idxx  = $row->ordercode;//$_POST[ "ordr_idxx"  ];                             // 주문 번호
$buyr_name  = $row->name;//$_POST[ "buyr_name"  ];                             // 주문자 이름
$buyr_tel1  = str_replace("-","",$row->tel);//$_POST[ "buyr_tel1"  ];                             // 주문자 전화번호
$buyr_mail  = $row->email;//$_POST[ "buyr_mail"  ];                             // 주문자 E-Mail
$good_name  = str_replace(array(",","-"," "),"",$row->productname);//$_POST[ "good_name"  ];                             // 상품 정보
$comment    = "";//$_POST[ "comment"    ];                             // 비고
/* = -------------------------------------------------------------------------- = */

$corp_type     = "0";//$_POST[ "corp_type"      ];                      // 사업장 구분
$corp_tax_type = "TG01";//$_POST[ "corp_tax_type"  ];                      // 과세/면세 구분
$corp_tax_no   = $tax_cnum;//$_POST[ "corp_tax_no"    ];                      // 발행 사업자 번호
$corp_nm       = $tax_cname;//$_POST[ "corp_nm"        ];                      // 상호
$corp_owner_nm = $tax_cowner;//$_POST[ "corp_owner_nm"  ];                      // 대표자명
$corp_addr     = $tax_caddr;//$_POST[ "corp_addr"      ];                      // 사업장 주소
$corp_telno    = str_replace("-","",$tax_ctel);//$_POST[ "corp_telno"     ];                      // 사업장 대표 연락처
/* = -------------------------------------------------------------------------- = */
$tr_code    = $row->tr_code;//$_POST[ "tr_code"    ];                             // 발행용도
$id_info    = $row->id_info;//$_POST[ "id_info"    ];                             // 신분확인 ID
$amt_tot    = $row->amt1;//$_POST[ "amt_tot"    ];                             // 거래금액 총 합
$amt_sup    = $row->amt2;//$_POST[ "amt_sup"    ];                             // 공급가액
$amt_svc    = $row->amt3;//$_POST[ "amt_svc"    ];                             // 봉사료
$amt_tax    = $row->amt4;//$_POST[ "amt_tax"    ];                             // 부가가치세
/* = -------------------------------------------------------------------------- = */
