...
if($type=="exit") {
	if($_data->memberout_type=="N") {
		echo "<html><head><title></title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=euc-kr\"></head><body onload=\"alert('회원탈퇴를 하실 수 없습니다.\\n\\n쇼핑몰 운영자에게 문의하시기 바랍니다.');history.go(-1)\"></body></html>";exit;
	}
...
if($type=="exit") {
	...
	if ($row=mysql_fetch_object($result)) {
		if($row->member_out=="Y") {
			echo "<html><head><title></title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=euc-kr\"></head><body onload=\"alert('회원 아이디가 존재하지 않습니다.');history.go(-1)\"></body></html>";exit;
		}
		$exitname=$row->name;
		$exitemail=$row->email;
...
if($ssllogintype!="ssl") {
	...
	if($row=mysql_fetch_object($result)) {
		...
		echo "<html><head><title></title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=euc-kr\"></head><body onload=\"alert('아이디 또는 비밀번호가 틀립니다.');history.go(".$history.");\"></body></html>";exit;
	}
	mysql_free_result($result);
}
...
if($row=mysql_fetch_object($result)) {
	...
	if($ssllogintype!="ssl") {
		echo "<html><head><title></title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=euc-kr\"></head><body onload=\"alert('비밀번호가 틀립니다.');history.go(".$history.")\"></body></html>";exit;
	} else {
