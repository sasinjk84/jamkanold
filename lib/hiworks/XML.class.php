...
class XMLTag
{
	...
	*/
	...
	function AddChild($name, $attrs, $parents, $cleanTagName = true)
	{
		...
		$child->isEmptyTag = $this->isEmptyTag;
	...
	function GetXML()
	{
		...
		//Otherwise...
			...
			elseif(!empty($this->tagData))
				$out .= '>'.replace_xml_data($this->tagData);
...
function replace_xml_data($data) {
	return strtr($data, array('&' => '&amp;', '<' => '&lt;', '>' => '&gt;'));
}

function array_to_xml($array_value) {
	$php_version = explode('.', PHP_VERSION);
	if($php_version[0] < 5) {
		return array_to_xml_4($array_value);
	}

	current($array_value);
	$document_obj = new DOMDocument('1.0', 'iso-8859-1');
	$parent_element = $document_obj->createElement(key($array_value));
	$document_obj->appendChild($parent_element);

	xml_append_child($array_value[key($array_value)], &$document_obj, &$parent_element);
	$return_xml_str = $document_obj->saveXML($document_obj->documentElement, LIBXML_NOEMPTYTAG);

	return $return_xml_str;
}
function xml_append_child($array_value, &$document_obj, &$parent_element, $parent_key='') {
	foreach($array_value as $key => $value) {
		if(is_array($value)) {
			if(isset($value[0])) {
				xml_append_child($value, &$document_obj, &$parent_element, $key);
				continue;
			}
			if(is_int($key)) {
				$key = $parent_key;
			...
			$array_parent_element = $document_obj->createElement($key);
			$parent_element->appendChild($array_parent_element);

			xml_append_child($value, &$document_obj, &$array_parent_element, $key);
		} else {
			$now_element = $document_obj->createElement($key, replace_xml_data($value));
			$parent_element->appendChild($now_element);
		}
	}
}

function array_to_xml_4($array_value) {
	current($array_value);
	$xml_str = '<?xml version="1.0" encoding="UTF-8"?>';
	$document_obj = new XMLTag(key($array_value));
	$document_obj->isEmptyTag = false;

	xml_append_child_4($array_value[key($array_value)], &$document_obj, &$document_obj, key($array_value));
	$return_xml_str = $xml_str.$document_obj->GetXML(true);

	return $return_xml_str;
}
function xml_append_child_4($array_value, &$document_obj, &$parent_element, $parent_key='') {
	foreach($array_value as $key => $value) {
		if(is_array($value)) {
			if(isset($value[0])) {
				xml_append_child_4($value, &$document_obj, &$parent_element, $key);
				continue;
			}
			if(is_int($key)) {
				$key = $parent_key;
			...
			$parent_element->AddChild($key, array(), $parent_element->tagParents);
			xml_append_child_4($value, &$parent_element, &$parent_element->{$key}[count($parent_element->{$key})-1], $key);
		} else {
			$parent_element->AddChild($key, array(), $parent_element->tagParents);
			$parent_element->{$key}[count($parent_element->{$key})-1]->tagData = $value;
		}
	}
}
